...
if (!$dbExists) {
    if ($mode != modInstall::MODE_NEW) {
        ...
        $xpdo = $install->_connect(
                $install->settings->get('database_type') . ':server=' . $install->settings->get('database_server')
...
$dbCollation = 'Latin1_General_100_BIN2';
//if ($dbExists) {
...
//}
//unset($stmt);
$data['collations'] = array('Latin1_General_100_BIN2');
