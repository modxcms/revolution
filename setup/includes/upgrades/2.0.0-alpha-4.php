...
$table = $this->install->xpdo->getTableName($class);
$description = sprintf($this->install->lexicon['add_column'],'namespace',$table);
$sql = "ALTER TABLE {$table} ADD COLUMN `namespace` VARCHAR(40) NOT NULL DEFAULT 'core'";
$this->processResults($class,$description,$sql);
$description = sprintf($this->install->lexicon['add_column'],'editedon',$table);
$sql = "ALTER TABLE {$table} ADD COLUMN `editedon` TIMESTAMP ON UPDATE CURRENT_TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP";
$this->processResults($class,$description,$sql);
$description = sprintf($this->install->lexicon['add_column'],'area',$table);
