<?php
/**
 * Japanese Preload Lexicon Topic for Revolution setup
 *
 * @package setup
 * @subpackage lexicon
 * @author KUROI Enogu http://twitter.com/enogu
 */
$_lang['preload_err_cache'] = '[[+path]]cache ディレクトリーをPHPプロセスから書き込める状態にしてください。';
$_lang['preload_err_core_path'] = 'setup/includes/config.core.php を開き、MODX_CORE_PATHの値を修正してください。このパラメーターはcoreディレクトリーの場所を指している必要があります。';
$_lang['preload_err_mysql'] = 'MySQL拡張モジュールが発見できませんでした。';
$_lang['preload_err_pdo'] = 'PDO拡張モジュールが発見できませんでした。';
$_lang['preload_err_pdo_mysql'] = 'PDO拡張モジュールのMySQLドライバーが発見できませんでした。';
