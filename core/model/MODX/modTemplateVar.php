...
class modTemplateVar extends modElement
{
    ...
    public function prepareOutput($value, $resourceId = 0)
    {
        ...
        $mTypes = $this->xpdo->getOption('manipulatable_url_tv_output_types', null, 'Image,File,image,file');
        $mTypes = explode(',', $mTypes);
        if (!empty($value) && in_array($this->get('type'), $mTypes)) {
            $context = !empty($resourceId)
                ? $this->xpdo->getObject('modResource', $resourceId)->get('context_key')
            ...
            if (!empty($sourceCache) && !empty($sourceCache['class_key'])) {
                ...
                $classKey = in_array($sourceCache['class_key'], $coreSourceClasses)
                    ? 'sources.' . $sourceCache['class_key']
                    : $sourceCache['class_key'];
                if ($this->xpdo->loadClass($classKey)) {
                    /** @var modMediaSource $source */
                    $source = $this->xpdo->newObject($sourceCache['class_key']);
                    if ($source) {
    ...
    public function getRender($params, $value, array $paths, $method, $resourceId = 0, $type = 'text')
    {
        ...
        if ($className = $this->checkForRegisteredRenderMethod($type, $method)) {
            ...
            $deprecatedClassName = $method == 'input'
                ? 'MODX\modTemplateVarInputRenderDeprecated'
                : 'MODX\modTemplateVarOutputRenderDeprecated';
            $render = new $deprecatedClassName($this);

            foreach ($paths as $path) {
                $className = 'MODX\\' . str_replace([MODX_CORE_PATH . 'model/MODX/', '/'], ['', '\\'], $path) . $type;
            ...
            if (empty($output)) {
                $className = 'MODX\Processors\Element\Tv\Renders\Mgr\\' . ucfirst($method) . '\Text';
                if (class_exists($className)) {
                    $render = new $className($this);
                    $output = $render->render($value, $params);
                }
    ...
    {
        $v = false;
        if (!isset($this->xpdo->tvRenders)) $this->xpdo->tvRenders = [];
    ...
    public function getRenderDirectories($event, $subdir)
    {
        ...
        $renderPath = $this->xpdo->getOption('processors_path') . 'Element/Tv/Renders/' . ucfirst($context) . '/' . ucfirst($subdir) . '/';
        $renderDirectories = [
            $renderPath,
            $this->xpdo->getOption('processors_path') . 'Element/Tv/Renders/' .
            ($subdir == 'Input' ? 'Mgr' : 'Web') . '/' . ucfirst($subdir) . '/',
        ];
    ...
    public function processInheritBinding($default = '', $resourceId = null)
    {
        ...
        } elseif ($this->xpdo->resource instanceof modResource) {
