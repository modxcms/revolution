...
class modTransportPackage extends xPDOObject
{
                ...
                if ($sourceFile) {
                    ...
                    if (!$transferred) { /* if no transport zip, attempt to get it */
                        if (!$transferred = $this->transferPackage($sourceFile, $packageDir)) {
                            $this->xpdo->log(xPDO::LOG_LEVEL_ERROR, $this->xpdo->lexicon('package_err_transfer', [
                    ...
                    if ($transferred) {
                        if ($state < 0) {
                            ...
                            $state = is_dir($packageDir . $targetDir) ? $this->get('state')
    ...
    public function install(array $options = [])
    {
        ...
        if ($this->getTransport()) {
            ...
            $attributes[xPDOTransport::PACKAGE_ACTION] = $this->previousVersionInstalled()
                ? xPDOTransport::ACTION_UPGRADE : xPDOTransport::ACTION_INSTALL;
    ...
    public function uninstall(array $options = [])
    {
        ...
        if ($this->package) {
            ...
            if ($this->package->uninstall($attributes)) {
                ...
                $this->xpdo->log(xPDO::LOG_LEVEL_ERROR, $this->xpdo->lexicon('package_err_uninstall', [
    ...
    public function transferPackage($sourceFile, $targetDir)
    {
        ...
        if (is_dir($targetDir) && is_writable($targetDir)) {
            ...
            $source = $this->get('service_url') . $sourceFile . (strpos($sourceFile, '?') !== false ? '&'
        ...
        foreach ($dependencies as $package => $constraint) {
            ...
            switch (strtolower($package)) {
                ...
                default:
                    ...
                    $latest = $this->xpdo->getObject('transport.modTransportPackage', $latestQuery);
                    if ($latest) {
    ...
    {
        $this->parseSignature();
        $count = $this->xpdo->getCount('transport.modTransportPackage', [["UCASE({$this->xpdo->escape('package_name')}) LIKE UCASE({$this->xpdo->quote($this->identifier)})"], 'installed:IS NOT' => null, 'signature:!=' => $this->get('signature')]);
