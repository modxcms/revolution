...
class Remove extends modProcessor
{
    ...
    public function initialize()
    {
        ...
        if (in_array($this->getProperty('force'), ['true', 1, true])) $this->setProperty('force', true);
    ...
    {
        $this->modx->log(MODX::LOG_LEVEL_INFO, $this->modx->lexicon('package_remove_info_gpack'));
        ...
        if (file_exists($transportZip) && file_exists($transportDir)) {
            ...
            if ($this->package->removePackage($this->getProperty('force')) == false) {
                ...
                return $this->failure($this->modx->lexicon('package_err_remove', ['signature' => $this->package->getPrimaryKey()]));
            }
        } else {
