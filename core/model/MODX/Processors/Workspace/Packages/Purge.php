...
class Purge extends modProcessor
{
    ...
    public function initialize()
    {
        ...
        if (empty($this->packageName)) {
            $this->modx->log(MODX::LOG_LEVEL_ERROR, $this->modx->lexicon('packagename_err_ns'));

            return $this->failure(MODX::LOG_LEVEL_ERROR, $this->modx->lexicon('packagename_err_ns'));
        }
        ...
        if (empty($this->packages)) {
            $this->modx->log(MODX::LOG_LEVEL_ERROR, $this->modx->lexicon('packagename_err_nf'));

            return $this->failure(MODX::LOG_LEVEL_ERROR, $this->modx->lexicon('packagename_err_nf'));
        }
    ...
    {
        $this->modx->log(MODX::LOG_LEVEL_INFO, $this->modx->lexicon('packages_purge_info_gpurge', ['signature' => $package->signature]));
        ...
        if (file_exists($transportZip) && file_exists($transportDir)) {
            ...
            if ($package->remove() == false) {
                $this->modx->log(MODX::LOG_LEVEL_ERROR, $this->modx->lexicon('package_err_remove', ['signature' => $package->getPrimaryKey()]));
