...
class GetNodes extends modProcessor
{
    ...
    public function getInCategoryNodes(array $map)
    {
        ...
        foreach ($elements as $element) {
            ...
            $nodes[] = [
                ...
                'name' => $name,
                'cls' => implode(' ', $class),
                'iconCls' => 'icon ' . ($element->get('icon') ? $element->get('icon')
                        : ($element->get('static') ? 'icon-file-text-o' : 'icon-file-o')),
    ...
    public function getTypeNodes(array $map)
    {
        ...
        ');
        $c->leftJoin($elementClassKey, $elementClassKey, $this->modx->getSelectColumns($elementClassKey, $elementClassKey, '', ['category']) . ' = ' . $this->modx->getSelectColumns('modCategory', 'modCategory', '', ['id']));
        $c->leftJoin('modCategory', 'Children');
        $c->where([
