        ...
        while ($partition && $result) {
            switch ($partition) {
                case 'auto_publish':
                    ...
                    $this->modx->log(MODX::LOG_LEVEL_INFO, "-> " . $this->modx->lexicon('refresh_published', ['num' => $result['published']]));
                    $this->modx->log(MODX::LOG_LEVEL_INFO, "-> " . $this->modx->lexicon('refresh_unpublished', ['num' => $result['unpublished']]));
                    break;
                case 'context_settings':
                    $this->modx->log(MODX::LOG_LEVEL_INFO, $this->modx->lexicon('refresh_context_settings'));
                    foreach ($result as $ctxKey => $ctxResult) {
                        $this->modx->log(MODX::LOG_LEVEL_INFO, "-> " . $ctxKey . ": " . $this->modx->lexicon('refresh_' . ($ctxResult
                                    ? 'success' : 'failure')));
                    }
                    break;
                case 'paths':
                ...
                default:
                    if (is_bool($result)) {
                        $this->modx->log(MODX::LOG_LEVEL_INFO, $this->modx->lexicon('refresh_' . $partition) . ': ' . $this->modx->lexicon('refresh_' . ($result
                                    ? 'success' : 'failure')));
