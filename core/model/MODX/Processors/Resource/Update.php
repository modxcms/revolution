...
class Update extends modObjectUpdateProcessor
{
    ...
    public static function getInstance(MODX &$modx, $className, $properties = [])
    {
        ...
        $classKey = !empty($properties['class_key']) ? $properties['class_key']
            : ($object ? $object->get('class_key') : 'modDocument');
    ...
    public function beforeSet()
    {
        ...
        $this->workingContext = $this->modx->getContext($this->getProperty('context_key', $this->object->get('context_key')
            ? $this->object->get('context_key') : 'web'));
    ...
    public function checkParentContext()
    {
        $parent = $this->getProperty('parent', null);
        if ($this->object->get('parent') != $parent) {
            $this->oldParent = $this->modx->getObject('modResource', ['id' => $this->object->get('parent')]);
            ...
            if ($this->newParent && $this->newParent->get('context_key') !== $this->object->get('context_key')) {
                ...
                if ($this->object->get('id') == $this->oldContext->getOption('site_start')) {
                    $this->addFieldError('parent', $this->modx->lexicon('resource_err_move_sitestart'));
    ...
    public function checkForUnPublishOnSiteStart()
    {
        ...
        } elseif ($this->isSiteStart && (!empty($publishDate) || !empty($unPublishDate))) {
    ...
    public function fixParents()
    {
        ...
        if (!empty($this->oldParent)) {
            $oldParentChildrenCount = $this->modx->getCount('modResource', ['parent' => $this->oldParent->get('id')]);
