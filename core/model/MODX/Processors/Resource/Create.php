...
class Create extends modObjectCreateProcessor
{
    ...
    public static function getInstance(MODX &$modx, $className, $properties = [])
    {
        ...
        $modx->newObject($classKey);

        if (!in_array($classKey, ['modDocument', 'modResource', ''])) {
    ...
    public function initialize()
    {
        ...
        $classKey = $this->getProperty('class_key', 'MODX\modDocument');
        $this->classKey = !empty($classKey)
            ? $classKey
            : 'MODX\modDocument';
    ...
    public function setFieldDefaults()
    {
        ...
        $scriptProperties['template'] = !isset($scriptProperties['template'])
            ? (integer)$this->workingContext->getOption('default_template', 0) : (integer)$scriptProperties['template'];
        $scriptProperties['hidemenu'] = !isset($scriptProperties['hidemenu'])
            ? (integer)$this->workingContext->getOption('hidemenu_default', 0)
            : (empty($scriptProperties['hidemenu']) ? 0 : 1);
        $scriptProperties['isfolder'] = empty($scriptProperties['isfolder']) ? 0 : 1;
        $scriptProperties['richtext'] = !isset($scriptProperties['richtext'])
            ? (integer)$this->workingContext->getOption('richtext_default', 1)
            : (empty($scriptProperties['richtext']) ? 0 : 1);
        $scriptProperties['donthit'] = empty($scriptProperties['donthit']) ? 0 : 1;
        $scriptProperties['published'] = !isset($scriptProperties['published'])
            ? (integer)$this->workingContext->getOption('publish_default', 0)
            : (empty($scriptProperties['published']) ? 0 : 1);
        $scriptProperties['cacheable'] = !isset($scriptProperties['cacheable'])
            ? (integer)$this->workingContext->getOption('cache_default', 1)
            : (empty($scriptProperties['cacheable']) ? 0 : 1);
        $scriptProperties['searchable'] = !isset($scriptProperties['searchable'])
            ? (integer)$this->workingContext->getOption('search_default', 1)
            : (empty($scriptProperties['searchable']) ? 0 : 1);
        $scriptProperties['content_type'] = !isset($scriptProperties['content_type'])
            ? (integer)$this->workingContext->getOption('default_content_type', 1)
            : (integer)$scriptProperties['content_type'];
        ...
        if (isset($scriptProperties['published'])) {
            ...
            $scriptProperties['publishedby'] = $scriptProperties['published'] ? !empty($scriptProperties['publishedby'])
                ? $scriptProperties['publishedby'] : $this->modx->user->get('id') : 0;
        }
    ...
    public function checkIfParentIsContext()
    {
        ...
        if (!empty($parent) && !is_numeric($parent)) {
            $ctxCnt = $this->modx->getCount('modContext', ['key' => $parent]);
            if ($ctxCnt > 0) {
    ...
    public function setMenuIndex()
    {
        ...
        if (!empty($autoMenuIndex) && empty($menuIndex)) {
            $menuIndex = $this->modx->getCount('modResource', [
    ...
    public function saveResourceGroups()
    {
        ...
        if ($resourceGroups !== null) {
            $resourceGroups = json_decode($resourceGroups, true);
