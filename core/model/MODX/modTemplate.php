...
class modTemplate extends modElement
{
     ...
     */
    public static function listTemplateVars(modTemplate &$template, array $sort = ['name' => 'ASC'], $limit = 0, $offset = 0, array $conditions = [])
    {
    ...
    public function process($properties = null, $content = null)
    {
        ...
        if (!$this->_processed) {
            ...
            if (is_string($this->_output) && !empty($this->_output)) {
                ...
                $this->xpdo->parser->processElementTags($this->_tag, $this->_output, false, false, '[[', ']]', [], $maxIterations);
            }
            $this->filterOutput();
    ...
    public function & getMany($alias, $criteria = null, $cacheFlag = true)
    {
        if (($alias === 'TemplateVars' || $alias === 'modTemplateVar') && ($criteria === null || strtolower($criteria) === 'all')) {
            ...
            $c->select(['value' => $this->xpdo->getSelectColumns('modTemplateVar', 'modTemplateVar', '', ['default_text'])]);
