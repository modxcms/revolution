...
class modResource extends modAccessibleSimpleObject implements modResourceInterface
{
    ...
    public static function filterPathSegment(&$xpdo, $segment, array $options = [])
    {
        ...
        switch ($translit) {
            ...
            case 'iconv':
                ...
                $segment = iconv($mbext ? mb_detect_encoding($segment)
                    : $charset, $charset . '//TRANSLIT//IGNORE', $segment);
                break;
            default:
                ...
                if ($xpdo instanceof MODX) {
                    ...
                    if ($translit = $xpdo->getService('translit', $translitClass, $translitClassPath, $options)) {
                        $segment = $translit->translate($segment, $translit);
                    }
                }
                break;
        }
    ...
    public function addLock($user = 0, array $options = [])
    {
        ...
        if ($this->xpdo instanceof MODX) {
            ...
            if (empty($lockedBy) || ($lockedBy == $user)) {
                ...
                $this->xpdo->registry->locks->send('/resource/', [md5($this->get('id')) => $user], ['ttl' => $this->xpdo->getOption('lock_ttl', $options, 360)]);
                $locked = true;
    ...
    public function getAliasPath($alias = '', array $fields = [])
    {
        ...
        if (empty($fields['uri_override']) || empty($fields['uri'])) {
            ...
            if (!empty($fields['content_type']) && $contentType = $this->xpdo->getObject('MODX\modContentType', $fields['content_type'])) {
            ...
            if ($workingContext->getOption('use_alias_path', false)) {
                ...
                $parentResources = [];
                $query = $this->xpdo->newQuery('MODX\modResource');
                $query->select($this->xpdo->getSelectColumns('MODX\modResource', '', '', ['parent', 'alias', 'alias_visible', 'uri', 'uri_override']));
                ...
                while ($currResource) {
                    ...
                    // path.
                    if (!isset($currResource['alias_visible']) || $currResource['alias_visible'] == 1) {
    ...
    public function duplicate(array $options = [])
    {
        ...
        $newName = !empty($options['newName'])
            ? $options['newName']
            : ($prefixDuplicate ? $this->xpdo->lexicon('duplicate_of', [
    ...
    public function leaveGroup($resourceGroupPk)
    {
        if (!is_object($resourceGroupPk) && !($resourceGroupPk instanceof modResourceGroup)) {
            ...
            $resourceGroup = $this->xpdo->getObject('modResourceGroup', $c);
            if (empty($resourceGroup) || !is_object($resourceGroup) || !($resourceGroup instanceof modResourceGroup)) {
                $this->xpdo->log(MODX::LOG_LEVEL_ERROR, __METHOD__ . ' - No resource group: ' . (is_object($resourceGroupPk)
                        ? $resourceGroupPk->get('name') : $resourceGroupPk));
        ...
        if (!$this->isMember($resourceGroup->get('name'))) {
            $this->xpdo->log(MODX::LOG_LEVEL_ERROR, __METHOD__ . ' - Resource ' . $this->get('id') . ' is not in resource group: ' . (is_object($resourceGroupPk)
                    ? $resourceGroupPk->get('name') : $resourceGroupPk));
    ...
    public function getResourceGroupNames()
    {
        ...
        $resourceGroups = $this->xpdo->getCollectionGraph('modResourceGroup', '{"ResourceGroupResources":{}}', ['ResourceGroupResources.document' => $this->get('id')]);
    ...
    public function getResourceTypeName()
    {
        ...
        if ($className == 'MODX\modDocument' || $className == 'modDocument') {
            $className = 'document';
        }
    ...
    public function getProperty($key, $namespace = 'core', $default = null)
    {
        ...
        return array_key_exists($namespace, $properties) && array_key_exists($key, $properties[$namespace])
            ? $properties[$namespace][$key] : $default;
    }
    ...
    public function getProperties($namespace = 'core')
    {
        ...
        return array_key_exists($namespace, $properties) ? $properties[$namespace] : [];
    }
    ...
    public function setProperty($key, $value, $namespace = 'core')
    {
        $properties = $this->get('properties');
        $properties = !empty($properties) ? $properties : [];
        if (!array_key_exists($namespace, $properties)) $properties[$namespace] = [];
