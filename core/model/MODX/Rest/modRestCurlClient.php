...
class modRestCurlClient extends modRestClient
{
            ...
            case 'POST':
                ...
                switch ($contentType) {
                    ...
                    case 'xml':
                        ...
                        $xml = modRestArrayToXML::toXML($params, !empty($options['rootNode']) ? $options['rootNode']
                            : 'request');
    ...
    public function setOptions($ch, array $options = [])
    {
        ...
        curl_setopt($ch, CURLOPT_TIMEOUT, !empty($options['curlopt_timeout']) ? $options['curlopt_timeout']
        ...
        curl_setopt($ch, CURLOPT_USERAGENT, !empty($options['curlopt_useragent']) ? $options['curlopt_useragent']
    ...
    public function setAuth($ch, array $options = [])
    {
        ...
        if (!empty($options[modRestClient::OPT_USERPWD])) {
            ...
            $auth = curl_setopt($ch, CURLOPT_USERPWD, !empty($options[modRestClient::OPT_USERPWD])
                ? $options[modRestClient::OPT_USERPWD] : 'username:password');
        }
    ...
    class modRestArrayToXML
    {
        ...
        public static function toArray($xml)
        {
            ...
            foreach ($children as $key => $node) {
                ...
                if (isset($arr[$key])) {
                    if (!is_array($arr[$key]) || $arr[$key][0] == null) $arr[$key] = [$arr[$key]];
                    $arr[$key][] = $node;
                } else {
                    $arr[$key] = $node;
                }
