...
class modManagerResponse extends modResponse
{
    ...
    public function loadControllerClass($prefixNamespace = true)
    {
        ...
        if (!class_exists($className)) {
            ...
            if (empty($classPath)) {
                ...
                $classPath = MODX_CORE_PATH . 'model/MODX/modManagerControllerDeprecated.php';
            }
    ...
    public function instantiateController($className, $getInstanceMethod = 'getInstance')
    {
        try {
            ...
            $this->modx->controller = call_user_func_array([$c, $getInstanceMethod], [&$this->modx, $className, $this->action]);
            $this->modx->controller->setProperties($c instanceof SecurityLoginManagerController ? $_POST
    ...
    {
        $className = $this->action['controller'] . (!empty($this->action['class_postfix'])
                ? $this->action['class_postfix'] : 'ManagerController');
    ...
    {
        $namespace = array_key_exists($this->namespace, $this->namespaces) ? $this->namespaces[$this->namespace]
            : $this->namespaces['core'];
        /* find context path */
