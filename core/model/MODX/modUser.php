...
use MODX\Mail\modPHPMailer;
use xPDO\xPDO;
use xPDO\Om\xPDOObject;
use MODX\Mail\modMail;
 ...
 * @property string $remote_data Used for storing remote data for authentication for a User
 ...
 * @property modUserSetting[] settings
 *
...
class modUser extends modPrincipal
{
    ...
    public function set($k, $v = null, $vType = '')
    {
        ...
        if (in_array($k, ['password', 'cachepwd'])) {
            $hashing = new Hashing\modHashing($this->xpdo);
            if (!$this->get('salt')) {
    ...
    public function passwordMatches($password, array $options = [])
    {
        ...
        return $this->get('password') === $hashedPassword;
    }
    ...
    public function getUserGroupSettings()
    {
        ...
        $query->innerJoin('modUserGroupMember', 'Member', ['Member.member' => $this->get('id'), 'UserGroup.id = Member.user_group']);
    ...
    public function getUserGroups()
    {
        ...
        if (isset($_SESSION["modx.user.{$id}.userGroups"]) && $this->xpdo->user->get('id') == $this->get('id')) {
            ...
            $memberGroups = $this->xpdo->getCollectionGraph('modUserGroup', '{"UserGroupMembers":{}}', ['UserGroupMembers.member' => $this->get('id')]);
            if ($memberGroups) {
     ...
     *
     * @return modUserGroup|xPDOObject|null
     */
     ...
     */
    public function generatePassword($length = 10, array $options = [])
    {
        $options = array_merge([
