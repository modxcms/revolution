 ...
 * @property string $extended A JSON array of extended properties that can be used to store custom fields for the User
 *
 * @see modUser
 * @package modx
 */
class modUserProfile extends xPDOSimpleObject
{
    ...
    public function save($cacheFlag = null)
    {
        ...
        if ($this->xpdo instanceof MODX) {
            $this->xpdo->invokeEvent('OnUserProfileBeforeSave', [
        ...
        if ($saved && $this->xpdo instanceof MODX) {
            $this->xpdo->invokeEvent('OnUserProfileSave', [
                'mode' => $isNew ? modSystemEvent::MODE_NEW : modSystemEvent::MODE_UPD,
                'userprofile' => &$this,
                'cacheFlag' => $cacheFlag,
            ]);
    ...
    {
        if ($this->xpdo instanceof MODX) {
            $this->xpdo->invokeEvent('OnUserProfileBeforeRemove', [
                'userprofile' => &$this,
                'ancestors' => $ancestors,
            ]);
        }
        ...
        if ($this->xpdo instanceof MODX) {
            $this->xpdo->invokeEvent('OnUserProfileRemove', [
                'userprofile' => &$this,
                'ancestors' => $ancestors,
            ]);
        }
