...
class modSessionHandler {
    ...
    public function write($id, $data) {
        $written= false;
        $gcMaxlifetime = $this->modx->context->getOption('session_gc_maxlifetime',null, @ini_get('session.gc_max_lifetime'));
        $cacheLifetime = $this->modx->context->getOption('cache_db_session_lifetime',null, intval($gcMaxlifetime / 4));
    ...
    public function gc($max) {
        $max = (integer) $this->modx->context->getOption('session_gc_maxlifetime', array(), $max);
        $maxtime= time() - $max;
    ...
    protected function _getSession($id, $autoCreate= false) {
        $session= $this->modx->getObject('modSession', array('id' => $id), $this->modx->context->getOption('cache_db_session', array(), false));
