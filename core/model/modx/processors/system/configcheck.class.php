 ...
 */
class modSystemConfigCheck extends modProcessor
{
    protected $warnings = [];
    ...
    protected function getResource($criteria)
    {
        ...
        if ($c->prepare() && $c->stmt->execute()) {
            if ($row = $c->stmt->fetch(PDO::FETCH_ASSOC)) {
                $resource = $row;
            }
        }
    ...
    public function checkSystem()
    {
        ...
        $compare = version_compare($version, $require);
        if ($compare == -1) {
    ...
    public function checkResources()
    {
        ...
        if (!$auth) {
            $this->warnings[$this->modx->lexicon('configcheck_unauthorizedpage_unavailable')] =
                $this->modx->lexicon('configcheck_unauthorizedpage_unavailable_msg');
        } elseif (!$auth['published']) {
            $this->warnings[$this->modx->lexicon('configcheck_unauthorizedpage_unpublished')] =
                $this->modx->lexicon('configcheck_unauthorizedpage_unpublished_msg');
        }

        $err = $this->getResource($this->modx->getOption('error_page', null, 1));
        if (!$err) {
            $this->warnings[$this->modx->lexicon('configcheck_errorpage_unavailable')] =
                $this->modx->lexicon('configcheck_errorpage_unavailable_msg');
        } elseif (!$err['published']) {
            $this->warnings[$this->modx->lexicon('configcheck_errorpage_unpublished')] =
                $this->modx->lexicon('configcheck_errorpage_unpublished_msg');
        }
    }
    ...
    public function checkSettings()
    {
        ...
        $tags = $this->modx->getCount('modSystemSetting', [
            'key' => 'allow_tags_in_post',
            'value' => 1,
        ]);
        if ($tags) {
            $this->warnings[$this->modx->lexicon('configcheck_allowtagsinpost_system_enabled')] =
                $this->modx->lexicon('configcheck_allowtagsinpost_system_enabled_msg');
        }

        $tags = $this->modx->getCount('modContextSetting', [
            'context_key:!=' => 'mgr',
            'key' => 'allow_tags_in_post',
            'value' => 1,
        ]);
        if ($tags) {
            $this->warnings[$this->modx->lexicon('configcheck_allowtagsinpost_context_enabled')] =
                $this->modx->lexicon('configcheck_allowtagsinpost_context_enabled_msg');
        }
    }

}

return 'modSystemConfigCheck';
