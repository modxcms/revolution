...
class modX extends xPDO {
    ...
    public function cleanAlias($alias, array $options = array()) {
        ...
        $mbext = function_exists('mb_strlen') && (boolean) $this->getOption('use_multibyte', false);
        $charset = strtoupper((string) $this->getOption('modx_charset', $options, 'UTF-8'));
        $delimiter = $this->getOption('friendly_alias_word_delimiter', $options, '-');
        $delimiters = $this->getOption('friendly_alias_word_delimiters', $options, '-_');
        $maxlength = (integer) $this->getOption('friendly_alias_max_length', $options, 0);
        $stripElementTags = (boolean) $this->getOption('friendly_alias_strip_element_tags', $options, true);
        $trimchars = $this->getOption('friendly_alias_trim_chars', $options, '/.' . $delimiters);
        $restrictchars = $this->getOption('friendly_alias_restrict_chars', $options, 'pattern');
        $restrictcharspattern = $this->getOption('friendly_alias_restrict_chars_pattern', $options, '/[\0\x0B\t\n\r\f\a&=+%#<>"~`@\?\[\]\{\}\|\^\'\\\\]/');
        $lowercase = (boolean) $this->getOption('friendly_alias_lowercase_only', $options, true);
        $translit = $this->getOption('friendly_alias_translit', $options, $iconv ? 'iconv' : 'none');
        $translitClass = $this->getOption('friendly_alias_translit_class', $options, 'translit.modTransliterate');

        /* strip html and optionally MODX element tags (stripped by default) */
        $alias = $this->stripTags($alias, '', $stripElementTags ? array() : null);
        ...
        if ($this->getService('lexicon','modLexicon')) {
            $alias = str_replace('&', $this->lexicon('and') ? ' ' . $this->lexicon('and') . ' ' : ' and ', $alias);
        }
        ...
        switch ($translit) {
            ...
            default:
                ...
                $translitClassPath = $this->getOption('friendly_alias_translit_class_path', $options, $this->getOption('core_path', $options, MODX_CORE_PATH) . 'components/');
                if ($this->getService('translit', $translitClass, $translitClassPath, $options)) {
                    $alias = $this->translit->translate($alias, $translit);
                }
                break;
        }
        ...
        if ($maxlength > 0 && $length > $maxlength) {
            ...
            $alias = trim($alias, $trimchars);
            $this->log(modX::LOG_LEVEL_INFO, "alias after maxlength applied = {$alias}");
        }
