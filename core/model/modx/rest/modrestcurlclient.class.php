 ...
 * @subpackage rest
 */
if (!class_exists('modRestCurlClient')) {
    class modRestCurlClient extends modRestClient
    {
                ...
                case 'POST':
                    ...
                    switch ($contentType) {
                        ...
                        case 'xml':
                            ...
                            $xml = modRestArrayToXML::toXML($params, !empty($options['rootNode'])
                                    ? $options['rootNode']
                                    : 'request');
        ...
        public function setOptions($ch, array $options = array())
        {
            ...
            curl_setopt($ch, CURLOPT_RETURNTRANSFER, !empty($options['curlopt_returntransfer'])
                    ? $options['curlopt_returntransfer']
                    : 1);
            /* we dont want header gruft */
            curl_setopt($ch, CURLOPT_HEADER, !empty($options['curlopt_header'])
            ...
            curl_setopt($ch, CURLOPT_NOBODY, !empty($options['curlopt_nobody'])
                    ? $options['curlopt_nobody']
                    : 0);
            /* attempt to retrieve the modification date of the remote document for use with curl_getinfo() */
            curl_setopt($ch, CURLOPT_FILETIME, !empty($options['curlopt_filetime'])
                    ? $options['curlopt_filetime']
                    : 0);
            /* default timeout to 30 seconds */
            curl_setopt($ch, CURLOPT_TIMEOUT, !empty($options['curlopt_timeout'])
                    ? $options['curlopt_timeout']
                    : $this->config[modRestClient::OPT_TIMEOUT]);
            /* disable verifypeer since it's not helpful on most environments */
            curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, !empty($options['curlopt_ssl_verifypeer'])
                    ? $options['curlopt_ssl_verifypeer']
                    : 0);
            /* send a useragent to allow proper responses */
            curl_setopt($ch, CURLOPT_USERAGENT, !empty($options['curlopt_useragent'])
            ...
            curl_setopt($ch, CURLOPT_FORBID_REUSE, !empty($options['curlopt_forbid_reuse'])
                    ? $options['curlopt_forbid_reuse']
                    : 0);
            /* force the use of a new connection instead of a cached one */
            curl_setopt($ch, CURLOPT_FRESH_CONNECT, !empty($options['curlopt_fresh_connect'])
                    ? $options['curlopt_fresh_connect']
                    : 0);
            ...
            if (empty($safeMode) && empty($openBasedir)) {
                curl_setopt($ch, CURLOPT_FOLLOWLOCATION, !empty($options['curlopt_followlocation'])
        ...
        public function setAuth($ch, array $options = array())
        {
            ...
            if (!empty($options[modRestClient::OPT_USERPWD])) {
                $options[modRestClient::OPT_AUTHTYPE] =
                    $this->modx->getOption(modRestClient::OPT_AUTHTYPE, $options, 'BASIC');
                ...
                $auth = curl_setopt($ch, CURLOPT_USERPWD, !empty($options[modRestClient::OPT_USERPWD])
                        ? $options[modRestClient::OPT_USERPWD]
                        : 'username:password');
            }
        ...
        public function setProxy($ch, array $options = array())
        {
            ...
            if (!empty($proxyHost)) {
                ...
                if (!empty($proxyUserpwd)) {
                    ...
                    $proxyAuthType = $proxyAuthType == 'NTLM'
                        ? CURLAUTH_NTLM
                        : CURLAUTH_BASIC;
