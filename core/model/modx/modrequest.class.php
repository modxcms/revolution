...
class modRequest {
    ...
    public function handleRequest() {
        ...
        if (!$this->modx->checkSiteStatus()) {
            ...
            if (!$this->modx->context->getOption('site_unavailable_page',null,1)) {
                ...
                $this->modx->resourceIdentifier = $this->modx->context->getOption('site_unavailable_page');
            }
        } else {
    ...
    public function getResourceMethod() {
        $method = '';
        if (isset ($_REQUEST[$this->modx->context->getOption('request_param_alias', null, 'q')]))
            $method = "alias";
        elseif (isset ($_REQUEST[$this->modx->context->getOption('request_param_id', null, 'id')])) {
            $method = "id";
        }
        return $method;
    }
    ...
    public function getResourceIdentifier($method) {
        ...
        switch ($method) {
            case 'alias' :
                $rAlias = $this->modx->context->getOption('request_param_alias',null,'q');
                $identifier = isset ($_REQUEST[$rAlias]) ? $_REQUEST[$rAlias] : $identifier;
                break;
            case 'id' :
                $rId = $this->modx->context->getOption('request_param_id',null,'id');
                $identifier = isset ($_REQUEST[$rId]) ? $_REQUEST[$rId] : $identifier;
                break;
            default :
                $identifier = $this->modx->context->getOption('site_start',null,1);
        }
        return $identifier;
    }
    ...
    public function _cleanResourceIdentifier($identifier) {
        ...
        elseif ($this->modx->context->getOption('friendly_urls',null,false)) {
            $containerSuffix = trim($this->modx->context->getOption('container_suffix',null,''));
            ...
            elseif ($this->modx->context->getOption('site_start',null,1) == $this->modx->aliasMap[$identifier]) {
                $this->modx->sendRedirect($this->modx->context->getOption('site_url'));
            } else {
                $this->modx->resourceMethod = 'alias';
            }
        } else {
    ...
    public function sanitizeRequest() {
        ...
        if ($this->modx->context->getOption('allow_tags_in_post',null,true)) {
            modX :: sanitize($_POST);
        } else {
    ...
    public function checkPublishStatus() {
        ...
        $timeNow= time() + $this->modx->context->getOption('server_offset_time',null,0);
    ...
    public function getParameters($keys = array(), $type = 'GET') {
        ...
        if (isset($this->parameters[$type])) {
            ...
            if ($type == 'GET') {
                ...
                $methodIdentifier = $this->modx->context->getOption('request_param_' . $method, null, $method);
            }
            if (is_array($keys)) {
                foreach ($keys as $key) {
