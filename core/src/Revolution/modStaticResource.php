    ...
    public function getSourceFile(array $options = [])
    {
        if (empty($this->_sourceFile)) {
            ...
            if (!file_exists($filename)) {
                $this->_sourcePath = $this->xpdo->getOption('resource_static_path', $options,
            ...
            if (file_exists($this->_sourceFile)) {
                ...
                if ($content === false) {
                    $this->xpdo->log(xPDO::LOG_LEVEL_ERROR,
                        "No content could be retrieved from source file: {$this->_sourceFile}");
                }
            } else {
    ...
    public function getFileContent($file, array $options = [])
    {
        ...
        if ($this->getOne('ContentType')) {
            ...
            if (!is_string($content)) {
                $this->xpdo->log(xPDO::LOG_LEVEL_ERROR,
                    "modStaticResource->getFileContent({$file}): Could not get content from file.");
            }
        }
