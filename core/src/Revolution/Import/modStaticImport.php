...
class modStaticImport extends modImport
{
    ...
    public function importFiles(
        ...
        foreach ($files as $id => $value) {
            ...
            if (is_array($value)) {
                ...
                if (!$resource->save()) {
                    $this->log("Could not import resource from {$filepath}: <br /><pre>" . print_r($resource->toArray('',
                            true), true) . '</pre>');
                } else {
                    $this->log($this->modx->lexicon('import_site_success'));
                    $this->importFiles($allowedfiles, $resource->get('id'), $filepath . $id, $value, $context, $class,
                        $basefilepath);
                }
            } else {
                // create resource
                ...
                if (!empty($allowedfiles) && !in_array($ext, $allowedfiles)) {
                    ...
                    if (!$resource->save()) {
                        $this->log($this->modx->lexicon('import_site_failed') . "Could not import resource {$file} from {$filepath}{$filename}: <br /><pre>" . print_r($resource->toArray('',
                                true), true) . '</pre>');
                    } else {
    ...
    public function getResourceAlias(& $resource, $alias, $parent, $context = 'web')
    {
        ...
        while ($duplicateId = $this->modx->findResource($fullAlias) && $iterations > 0) {
            $iterations--;
            $this->log($this->modx->lexicon('import_duplicate_alias_found', [
                'id' => $duplicateId,
                'alias' => $fullAlias,
            ]));
    ...
    public function toXPath($selector)
    {
        ...
        foreach ($selectors as &$selector) {
            ...
            // :button, :submit, etc
            $selector = preg_replace('/:(button|submit|file|checkbox|radio|image|reset|text|password)/',
            ...
            // div.foo
            $selector = preg_replace('/([\w\-]+)\.([\w\-]+)/', '\1[contains(concat(" ",@class," ")," \2 ")]',
                $selector);
            // .foo
