...
class modX extends App
{
    ...
    /**
     * @var \Slim\Collection modX configuration data.
     */
    public $config;
    /**
     * @var xPDO The xPDO instance used to access the configured data source.
     */
    public $xpdo;
    /**
    ...
    /**
     * @var int Unix timestamp indicating when the instance was initialized.
     */
    public $startTime;
    /**
     ...
     *
     * @param string $configPath An absolute filesystem path to look for the modX config file.
     * @param array $options modX options that can override configuration for the instance.
     * @param array $driverOptions PDO driver options that can be passed to the xPDO data layer.
    ...
    protected function configureContainer(ContainerBuilder $builder)
    {
        $builder->addDefinitions(array(
            ...
            'xpdo' => function(ContainerInterface $c) {
                ...
                $configPath= $config->get('configPath', MODX_CORE_PATH . 'config/');
    ...
    protected function _loadExtensionPackages($options = null) {
        $cache = $this->xpdo->call('modExtensionPackage', 'loadCache', array(&$this));
        if (!empty($cache)) {
            foreach ($cache as $package) {
                $package['table_prefix'] = !empty($package['table_prefix']) ? $package['table_prefix'] : null;
                $this->xpdo->addPackage($package['namespace'], $package['path'], $package['table_prefix']);
                if (!empty($package['service_name']) && !empty($package['service_class'])) {
                    $this->xpdo->getService($package['service_name'], $package['service_class'], $package['path']);
                }
            }
        }
    }
    ...
    public function getFullTableName( $table = '' ) {
        return $this->xpdo->getOption('dbname') .".". $this->xpdo->getOption( xPDO::OPT_TABLE_PREFIX ) . $table;
    }
