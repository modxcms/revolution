...
use PDO;use MODX\modContext;
use MODX\modContentType;
use MODX\modResource;
use MODX\modDocument;
...
class modStaticImport extends modImport
{
     ...
     */
    public function importFiles($allowedfiles, $parent, $filepath, $files, $context = 'web', $class = 'modStaticResource', $basefilepath = '', $elements = [])
    {
        if (!is_array($files))
            return;
        if ($parent > 0) {
