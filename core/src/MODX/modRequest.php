...
class modRequest
{
        ...
        if (!$this->modx->checkSiteStatus()) {
            ...
            if ($this->modx->resourceMethod == 'id' && $this->modx->getOption('friendly_urls', null, false) && $this->modx->getOption('request_method_strict', null, false)) {
                ...
                if (!empty($uri)) {
                    ...
                    $this->modx->sendRedirect($url, ['responseCode' => $_SERVER['SERVER_PROTOCOL'] . ' 301 Moved Permanently']);
                }
            }
        }
    ...
    public function getResource($method, $identifier, array $options = [])
    {
        ...
        if (is_array($cachedResource) && array_key_exists('resource', $cachedResource) && is_array($cachedResource['resource'])) {
            ...
            $name = $cachedResource['resourceClass'];
            $resource = new $name($this->modx);
            if ($resource) {
                ...
                if (isset($cachedResource['contentType'])) {
                    $contentType = $this->modx->newObject('MODX\modContentType');
                ...
                if (isset($cachedResource['resourceGroups'])) {
                    ...
                    foreach ($cachedResource['resourceGroups'] as $rGroupKey => $rGroup) {
                        $rGroups[$rGroupKey] = $this->modx->newObject('MODX\modResourceGroupResource', $rGroup);
                    }
                    $resource->addMany($rGroups);
                }
                if (isset($cachedResource['policyCache'])) $resource->setPolicies([$this->modx->context->get('key') => $cachedResource['policyCache']]);
                ...
                if ($resource instanceof modResource) {
                    if ($resource->get('context_key') !== $this->modx->context->get('key')) {
                        if (!$isForward || ($isForward && !$this->modx->getOption('allow_forward_across_contexts', $options, false))) {
                            if (!$this->modx->getCount('modContextResource', [$this->modx->context->get('key'), $resourceId])) {
                                return null;
                            }
                        }
                    }
    ...
    public function _cleanResourceIdentifier($identifier)
    {
        if (empty ($identifier)) {
            if ($this->modx->getOption('base_url', null, MODX_BASE_URL) !== strtok($_SERVER["REQUEST_URI"], '?')) {
                $this->modx->sendRedirect($this->modx->getOption('site_url', null, MODX_SITE_URL), ['responseCode' => $_SERVER['SERVER_PROTOCOL'] . ' 301 Moved Permanently']);
            }
            $identifier = $this->modx->getOption('site_start', null, 1);
            $this->modx->resourceMethod = 'id';
    ...
    {
        $className = 'MODX\Error\\' . $class;
        try {
            $this->modx->error = new $className($this->modx);
        } catch (\Error $e) {
            $this->modx->log(MODX::LOG_LEVEL_FATAL, 'Error handling class could not be loaded: ' . $e->getMessage());
        }
    }
    ...
    public function registerLogging(array $options = [])
    {
        if (isset($options['register']) && isset($options['topic'])) {
            if ($this->modx->getService('registry', 'registry.modRegistry')) {
                $register_class = isset($options['register_class']) ? $options['register_class']
    ...
    public function getAllActionIDs($namespace = '')
    {
        ...
        foreach ($actions as $action) {
            $key = ($action->get('namespace') == 'core' ? ''
                    : $action->get('namespace') . ':') . $action->get('controller');
            $actionList[$key] = $action->get('id');
        }

        // Also add old core actions for backwards compatibility
        $oldActions = ['browser', 'context',
     ...
     * @return array
     */
    public function getActionIDs(array $actions = [], $namespace = 'core')
    {
        $as = [];
        foreach ($actions as $action) {
