...
class modWebLink extends modResource implements modResourceInterface
{
    ...
    public function process()
    {
        ...
        if (!is_numeric($this->_content)) {
            $this->xpdo->getParser();
            $maxIterations = isset ($this->xpdo->config['parser_max_iterations'])
                ? intval($this->xpdo->config['parser_max_iterations']) : 10;
            $this->xpdo->parser->processElementTags($this->_tag, $this->_content, true, true, '[[', ']]', [], $maxIterations);
        }
        if (is_numeric($this->_content)) {
        ...
        $this->xpdo->sendRedirect($this->_output, ['responseCode' => $this->getProperty('responseCode', 'core', $_SERVER['SERVER_PROTOCOL'] . ' 301 Moved Permanently')]);
    }
