...
abstract class modRestController
{
    ...
    protected function process($success = true, $message = '', $object = [], $status = 200)
    {
        ...
        $this->responseStatus = empty($status)
            ? (empty($success)
                ? $this->getOption('defaultFailureStatusCode', 200)
                : $this->getOption('defaultSuccessStatusCode', 200)
            )
            : $status;
    }
    ...
    public function setDefault($k, $v, $useNotEmpty = false)
    {
        ...
        } elseif (array_key_exists($k, $this->properties)) {
            $isSet = true;
        }
        if (!$isSet) {
            $this->properties[$k] = $v;
    ...
    {
        if (empty($id)) {
            $this->failure($this->modx->lexicon('rest.err_field_ns', [
        ...
        if (empty($this->object)) {
            $this->failure($this->modx->lexicon('rest.err_obj_nf', [
    ...
    public function post()
    {
        ...
        if (!$this->object->{$this->postMethod}()) {
            ...
            if ($this->hasErrors()) {
                ...
                $this->failure($this->modx->lexicon('rest.err_class_save', [
                    'class_key' => $this->classKey,
                ]));
    ...
    public function delete()
    {
        ...
        if (!$this->object->{$this->deleteMethod}()) {
            ...
            $this->failure($this->modx->lexicon('rest.err_class_remove', [
                'class_key' => $this->classKey,
            ]));
