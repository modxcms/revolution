...
use xPDO\xPDO;use DirectoryIterator;
...
class modLexicon
{
        ...
        foreach ($topics as $topicStr) {
            ...
            if ($nspos === false) {
                foreach ($this->_paths as $namespace => $path) {
                    ...
                    if (is_array($entries)) {
                        if (!array_key_exists($defaultLanguage, $this->_lexicon)) $this->_lexicon[$defaultLanguage] = [];
                        $this->_lexicon[$defaultLanguage] = is_array($this->_lexicon[$defaultLanguage])
                            ? array_merge($this->_lexicon[$defaultLanguage], $entries) : $entries;
                    }
                }
                ...
                if (!is_array($entries)) {
                    if (is_string($entries) && !empty($entries)) $entries = json_decode($entries, true);
                ...
                if (is_array($entries)) {
                    ...
                    if (!array_key_exists($language, $this->_lexicon)) $this->_lexicon[$language] = [];
                    $this->_lexicon[$language] = is_array($this->_lexicon[$language])
                        ? array_merge($this->_lexicon[$language], $entries) : $entries;
                }
            }
        }
    }
     ...
     *
     * @return array|bool An array of lexicon entries in key - value pairs for the specified filter.
     */
    ...
    public function getTopicList($language = 'en', $namespace = 'core')
    {
        ...
        foreach (new DirectoryIterator($lexPath) as $topic) {
            if (in_array($topic, ['.', '..', '.svn', '.git', '_notes'])) continue;
    ...
    public function getLanguageList($namespace = 'core')
    {
        ...
        foreach (new DirectoryIterator($lexPath) as $language) {
            if (in_array($language, ['.', '..', '.svn', '.git', '_notes', 'country'])) continue;
