...
use xPDO\xPDO;
use MODX\Filters\modInputFilter;
use MODX\Filters\modOutputFilter;
use MODX\Sources\modMediaSourceInterface;
use MODX\Sources\modMediaSource;use PDO;
...
class modElement extends modAccessibleSimpleObject
{
    ...
    public function getFileContent(array $options = [])
    {
        ...
        if ($this->isStatic() && $sourceFile = $this->getSourceFile($options)) {
            /** @var modMediaSourceInterface $source */
            if ($source = $this->getSource()) {
    ...
    public function addPropertySet($propertySet)
    {
        ...
        if (!empty($propertySet)) {
            ...
            if (is_object($propertySet) && $propertySet instanceof modPropertySet) {
                if (!$this->isNew() && !$propertySet->isNew() && $this->xpdo->getCount('modElementPropertySet', ['element' => $this->get('id'), 'element_class' => $this->_class, 'property_set' => $propertySet->get('id')])) {
                    $added = true;
                } else {
    ...
    public function removePropertySet($propertySet)
    {
        ...
        if (!empty($propertySet)) {
            ...
            if (is_object($propertySet) && $propertySet instanceof modPropertySet) {
                $removed = $this->xpdo->removeObject('modElementPropertySet', ['element' => $this->get('id'), 'element_class' => $this->_class, 'property_set' => $propertySet->get('id')]);
            }
        }
    ...
    public function getSourceCache($contextKey = '', array $options = [])
    {
        ...
        if (!$cacheManager || !($cacheManager instanceof modCacheManager)) return [];
