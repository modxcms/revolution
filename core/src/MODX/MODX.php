...
class MODX extends xPDO
{
    ...
    public $context = null;
    /**
     * @var modContext[] An array of secondary contexts loaded on demand.
     */
    public $contexts = [];
    /**
    ...
    public $request = null;
    /**
     * @var modResponse|modConnectorResponse Represents a web response, providing helper methods for
    ...
    public $virtualDir;
    /**
     * @var Error\modErrorHandler|object An error_handler for the MODX instance.
     */
    public $errorHandler = null;
    /**
     * @var Error\modError An error response class for the request
     */
    public $error = null;
    /**
    ...
    /**
     * @var Registry\modRegistry $registry
     */
    public $registry;
    /**
     * @var Mail\modMail $mail
     */
    public $mail;
    /**
     * @var Rest\modRest|Rest\modRestClient $rest
     */
    public $rest;
    /**
     * @var Smarty\modSmarty $smarty
     */
    public $smarty;
    /**
    ...
    public function __construct($configPath = '', $options = null, $driverOptions = null)
    {
        try {
            ...
            $this->setPackage('MODX', MODX_CORE_PATH . 'src/');
        ...
        if (file_exists($configPath . MODX_CONFIG_KEY . '.inc.php') && include($configPath . MODX_CONFIG_KEY . '.inc.php')) {
            ...
            $data = array_merge(
                [
                    ...
                    xPDO::OPT_VALIDATOR_CLASS => 'MODX\Validation\modValidator',
            ...
            $primaryConnection = [
                ...
                'options' => [
                    xPDO::OPT_CONN_MUTABLE => isset($data[xPDO::OPT_CONN_MUTABLE])
                        ? (boolean)$data[xPDO::OPT_CONN_MUTABLE] : true,
                ],
    ...
    public function initialize($contextKey = 'web', $options = null)
    {
        if (!$this->_initialized) {
            ...
            $this->registry = new Registry\modRegistry($this);
    ...
    {
        $cache = $this->call('MODX\modExtensionPackage', 'loadCache', [&$this]);
        if (!empty($cache)) {
    ...
    public function getCacheManager($class = 'xPDO\Cache\xPDOCacheManager', $options = ['path' => XPDO_CORE_PATH, 'ignorePkg' => true])
    {
        if ($this->cacheManager === null) {
            ...
            $className = !class_exists($cacheManagerClass)
                ? $this->loadClass($cacheManagerClass, $options['path'], $options['ignorePkg'], true)
                : $cacheManagerClass;
            try {
                ...
                $this->log(MODX::LOG_LEVEL_FATAL, 'Could not initialize cache manager: ' . $e->getMessage());
            }
        }
     ...
     * in the process.
     *
     * @return modParser|object The modParser for this MODX instance.
     */
    public function getParser()
    {
        return $this->getService('parser', $this->getOption('parser_class', null, 'MODX\modParser'), $this->getOption('parser_class_path', null, ''));
    }
    ...
    public function getParentIds($id = null, $height = 10, array $options = [])
    {
        ...
        if ($id && $height > 0) {
            ...
            $resourceMap = !empty($context) && !empty($this->contexts[$context]->resourceMap)
                ? $this->contexts[$context]->resourceMap : $this->resourceMap;
        ...
        if ($validid = intval($id)) {
            ...
            if (empty($url) && ($context !== $this->context->get('key'))) {
                ...
                if ($context == '') {
                    ...
                    if ($stmt = $this->prepare("SELECT context_key FROM " . $this->getTableName('MODX\modResource') . " WHERE id = :id")) {
    ...
    {
        return $this->call('MODX\modResource', 'filterPathSegment', [&$this, $string, $options]);
    }
    ...
    public function findResource($uri, $context = '')
    {
        ...
        if (!empty($context) && (!empty($uri) || $uri === '0')) {
            ...
            if (!$resourceId && !$useAliasMap) {
                $query = $this->newQuery('MODX\modResource', ['context_key' => $context, 'uri' => $uri, 'deleted' => false]);
                $query->select($this->getSelectColumns('MODX\modResource', '', '', ['id']));
                $stmt = $query->prepare();
                if ($stmt) {
    ...
    {
        $this->getResponse()
            ->sendRedirect($url, $options, $type, $responseCode);
    }
    ...
    public function getUser($contextKey = '', $forceLoadSettings = false)
    {
        ...
        if ($this->user !== null && is_object($this->user)) {
            ...
            $this->user = $this->newObject('MODX\modUser');
            $this->user->fromArray([
                'id' => 0,
     ...
     *
     * @return \MODX\modUser or null if no user is authenticated.
     */
    public function getAuthenticatedUser($contextKey = '')
    {
        ...
        if ($contextKey && isset ($_SESSION['modx.user.contextTokens'][$contextKey])) {
            ...
            $user = $this->getObject('MODX\modUser', intval($_SESSION['modx.user.contextTokens'][$contextKey]), true);
            if ($user) {
     ...
     * @return modRequest
     */
    public function getRequest($class = 'MODX\modRequest', $path = '')
    {
        $requestClass = $this->getOption('modRequest.class', $this->config, $class);
        if (strpos($requestClass, '\\') === false) {
            $requestClass = $this->loadClass($requestClass, $path);
        }
        if ($this->request === null || !($this->request instanceof $requestClass)) {
            try {
                $this->request = new $requestClass($this);
            } catch (\Error $e) {
                $this->log(xPDO::LOG_LEVEL_FATAL, 'Could not load request class:' . $e->getMessage());
            }
        }
     ...
     * Defaults to the current MODX model path.
     *
     * @return modResponse|modConnectorResponse
     */
    public function getResponse($class = 'MODX\modResponse', $path = '')
    {
        $responseClass = $this->getOption('modResponse.class', $this->config, $class);
        if (strpos($responseClass, '\\') === false) {
            $responseClass = $this->loadClass($responseClass, $path);
        }
        if ($this->response === null || !($this->response instanceof $responseClass)) {
            try {
                $this->response = new $responseClass($this);
            } catch (\Error $e) {
                $this->log(xPDO::LOG_LEVEL_FATAL, 'Could not load response class:' . $e->getMessage());
            }
        }
        ...
        if (count($this->eventMap[$eventName])) {
            ...
            foreach ($this->eventMap[$eventName] as $pluginId => $pluginPropset) {
                ...
                if (isset($this->pluginCache[$pluginId])) {
                    ...
                    $plugin = $this->newObject('MODX\modPlugin');
                    ...
                    $plugin = $this->getObject('MODX\modPlugin', ['id' => intval($pluginId), 'disabled' => '0'], true);
                }
                if ($plugin && !$plugin->get('disabled')) {
                    $this->event->plugin =& $plugin;
                    $this->event->activated = true;
                    $this->event->activePlugin = $plugin->get('name');
                    $this->event->propertySet = (($pspos = strpos($pluginPropset, ':')) >= 1)
                        ? substr($pluginPropset, $pspos + 1)
                        : '';
    ...
    public function runProcessor($action = '', $scriptProperties = [], $options = [])
    {
        ...
        if (!empty($options['location'])) {
            $tmp = array_merge(explode('/', $options['location']), $tmp);
        }
        $className = 'MODX\Processors\\' . implode('\\', array_map('ucfirst', $tmp));
        if (class_exists($className)) {
            /** @var Processors\modProcessor $className */
            $className = call_user_func_array([$className, 'getInstance'], [&$this, $className, $scriptProperties]);
        ...
        $isClass = true;
        $processorsPath = isset($options['processors_path']) && !empty($options['processors_path'])
            ? $options['processors_path'] : $this->config['processors_path'];
        ...
        if (file_exists($processorFile)) {
            if (!isset($this->lexicon)) {
                $this->getService('lexicon', 'MODX\modLexicon');
            }
            if (!isset($this->error)) {
                $this->getService('error', 'MODX\Error\modError');
            }
            if ($isClass) {
            ...
            if (empty($processor)) {
                $processor = new Processors\modDeprecatedProcessor($this, $scriptProperties);
            }
     ...
     *
     * @return modManagerLog|xPDOObject The newly created modManagerLog object.
     */
    public function logManagerAction($action, $class_key, $item, $userId = null)
    {
        ...
        $ml = $this->newObject('MODX\modManagerLog');
    ...
    public function getContext($contextKey)
    {
        if (!isset($this->contexts[$contextKey])) {
            $this->contexts[$contextKey] = $this->getObject('MODX\modContext', ['key' => $contextKey]);
    ...
    public function getEventMap($contextKey)
    {
        ...
        if ($contextKey) {
            ...
            $pluginEventTbl = $this->getTableName('MODX\modPluginEvent');
            $eventTbl = $this->getTableName('MODX\modEvent');
            $pluginTbl = $this->getTableName('MODX\modPlugin');
            $propsetTbl = $this->getTableName('MODX\modPropertySet');
            $sql = "
                SELECT
                    Event.name AS event,
            ...
            if ($stmt && $stmt->execute()) {
                while ($ee = $stmt->fetch(PDO::FETCH_ASSOC)) {
                    $eventElementMap[$ee['event']][(string)$ee['pluginid']] = $ee['pluginid'] . (!empty($ee['propertyset'])
                            ? ':' . $ee['propertyset'] : '');
                }
            }
        }
    ...
    public function checkForLocks($id, $action, $type)
    {
        ...
        if ($au = $this->getObject('MODX\modActiveUser', ['action' => $action, 'internalKey:!=' => $id])) {
    ...
    public function addExtensionPackage($name, $path, array $options = [])
    {
        ...
        $extPackages = is_array($extPackages) ? $extPackages : json_decode($extPackages, true);
        $extPackages[$name] = $options;
        $extPackages['path'] = $path;

        /** @var modSystemSetting $setting */
        $setting = $this->getObject('MODX\modSystemSetting', [
            'key' => 'extension_packages',
        ]);
        if (empty($setting)) {
            $setting = $this->newObject('MODX\modSystemSetting');
        ...
        if ($this->context instanceof modContext) {
            ...
            if ($key === $this->context->get('key')) {
                ...
                if (!array_key_exists($key, $this->contexts) || !($this->contexts[$key] instanceof modContext)) {
                    $this->contexts[$key] = $this->newObject('MODX\modContext');
    ...
    protected function _initContext($contextKey, $regenerate = false, $options = null)
    {
        ...
        if (isset($this->contexts[$contextKey]) && $this->contexts[$contextKey] instanceof modContext) {
            ...
            $this->context = $this->newObject('MODX\modContext');
    ...
    protected function _initCulture($options = null)
    {
        ...
        $this->getService('lexicon', $this->getOption('lexicon_class', $options, 'MODX\modLexicon'), '', is_array($options)
            ? $options : []);
    ...
    protected function _initErrorHandler($options = null)
    {
        if ($this->errorHandler == null || !is_object($this->errorHandler)) {
            if ($ehClass = $this->getOption('error_handler_class', $options, 'MODX\Error\modErrorHandler', true)) {
    ...
    protected function _loadConfig()
    {
        ...
        if (empty($config)) {
            $config = [];
            if (!$settings = $this->getCollection('MODX\modSystemSetting')) {
                return false;
            }
            /** @var modSystemSetting $setting */
    ...
    protected function _log($level, $msg, $target = '', $def = '', $file = '', $line = '')
    {
        ...
        if (is_object($targetObj) && $targetObj instanceof modRegister) {
            if ($level === MODX::LOG_LEVEL_FATAL) {
                if (empty ($file)) $file = (isset ($_SERVER['PHP_SELF'])) ? $_SERVER['PHP_SELF']
            ...
            if ($level === MODX::LOG_LEVEL_FATAL) {
                ...
                if ($targetObj == 'FILE' && $cacheManager = $this->getCacheManager()) {
                    ...
                    $filepath = isset($targetOptions['filepath']) ? $targetOptions['filepath']
    ...
    public function loadClass($fqn, $path = '', $ignorePkg = false, $transient = false)
    {
        if (strpos($fqn, '\\') !== false && class_exists($fqn)) {
            ...
            if (class_exists($class)) {
                ...
                $this->log(MODX::LOG_LEVEL_DEBUG, "Could not find class {$fqn} converted to {$class}");
            }
        }
