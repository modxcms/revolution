...
use xPDO\xPDO;use PDOStatement;
...
class modContext extends modAccessibleObject
{
            ...
            if ($found) {
                ...
                if ($target === null && $scheme !== -1 && $scheme !== '') {
                    if ($scheme === 1 || $scheme === 0) {
                        ...
                        $isSecure = ($_SERVER['SERVER_PORT'] == $https_port || (isset($_SERVER['HTTPS']) && strtolower($_SERVER['HTTPS']) == 'on'))
                            ? 1 : 0;
                        if ($scheme != $isSecure) {
    ...
    public function getResourceURI($id)
    {
        if ($this->getOption('cache_alias_map') && isset($this->aliasMap)) {
            ...
            $query->select($this->xpdo->getSelectColumns('MODX\modResource', '', '', ['uri']));
