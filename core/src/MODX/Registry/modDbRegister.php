...
use MODX\MODX;
use MODX\Registry\DB\modDbRegisterQueue;
...
class modDbRegister extends modRegister
{
     ...
     *
     * @return modDbRegisterQueue|xPDOObject A reference to the new Queue object
     */
    protected function _initQueue($key, $options)
    {
        $queue = $this->modx->getObject('registry.db.modDbRegisterQueue', [
            'name' => $key,
        ]);
        if (!$queue) {
    ...
    {
        $result = $this->modx->removeCollection('registry.db.modDbRegisterMessage', ['topic' => $topic]);
    ...
    protected function _readMessage($obj, $remove = true)
    {
        ...
        if (is_object($obj) && !empty($obj->payload)) {
            ...
            if ($remove || ($obj->expires > 1 && $obj->expires < time())) {
                $this->modx->removeObject('registry.db.modDbRegisterMessage', ['topic' => $obj->topic, 'id' => $obj->id]);
            }
    ...
    public function send($topic, $message, array $options = [])
    {
        ...
        if ($queueId && $topicIdx !== false) {
            ...
            if (!$topicObj = $this->modx->getObject('registry.db.modDbRegisterTopic', ['queue' => $queueId, 'name' => $topic])) {
                    ...
                    if (is_scalar($msg) || is_array($msg) || is_object($msg)) {
                        switch ($messageType) {
                            ...
                            default :
                                ...
                                $expires = isset($options['ttl']) && intval($options['ttl'])
                                    ? time() + intval($options['ttl']) : 0;
                                ...
                                $messageObj = $this->modx->getObject('registry.db.modDbRegisterMessage', ['topic' => $topicObj->get('id'), 'id' => $msgKey]);
                                if (!$messageObj) {
