...
class modCacheManager extends xPDOCacheManager
{
    ...
    public function generateContext($key, array $options = [])
    {
        ...
        if (!$this->getOption('transient_context', $options, false)) {
            ...
            $options['cache_context_settings'] = array_key_exists('cache_context_settings', $results)
    ...
    public function getElementMediaSourceCache(modElement $element, $contextKey, array $options = [])
    {
        ...
        if (empty($sourceCache)) {
            ...
            $c->sortby($this->modx->getSelectColumns('sources.modMediaSourceElement', 'modMediaSourceElement', '', ['object']), 'ASC');
        ...
        if ($this->getOption('cache_resource', $options, true)) {
            if (is_object($obj) && $obj instanceof modResource && $obj->getProcessed() && $obj->get('cacheable') && $obj->get('id')) {
                ...
                if ($docGroups = $obj->getMany('ResourceGroupResources')) {
                    $groups = [];
                    /** @var modResourceGroupResource $docGroup */
        ...
        if ($c->prepare() && $c->stmt->execute()) {
            ...
            foreach ($namespaces as $namespace) {
                ...
                if ($namespace['name'] == 'core') {
                    ...
                    $namespace['path'] = $this->modx->call('modNamespace', 'translatePath', [&$this->modx, $namespace['path']]);
                    $namespace['assets_path'] = $this->modx->call('modNamespace', 'translatePath', [&$this->modx, $namespace['assets_path']]);
                }
        ...
        if ($c->prepare() && $c->stmt->execute()) {
            ...
            foreach ($extensionPackages as $extensionPackage) {
                ...
                if (empty($extensionPackage['path'])) {
                    $extensionPackage['path'] = $this->modx->call('modNamespace', 'translatePath', [&$this->modx, $extensionPackage['namespace_path']]);
                }
    ...
    public function refresh(array $providers = [], array &$results = [])
    {
        ...
        foreach ($providers as $partition => $partOptions) {
            ...
            if (!is_array($partOptions)) $partOptions = [];
            $partOptions = array_merge($partOptions, [xPDO::OPT_CACHE_KEY => $partKey, xPDO::OPT_CACHE_HANDLER => $partHandler]);
            switch ($partition) {
                case 'auto_publish':
    ...
    public function autoPublish(array $options = [])
    {
        ...
        if ($stmt) {
            ...
            if ($stmt->execute()) {
                ...
                $publishingResults['errors'][] = $this->modx->lexicon('cache_publish_event_error', ['info' => $stmt->errorInfo()]);
            }
        } else {
        ...
        if ($stmt) {
            ...
            if ($stmt->execute()) {
                ...
                $publishingResults['errors'][] = $this->modx->lexicon('cache_unpublish_event_error', ['info' => $stmt->errorInfo()]);
            }
        } else {
    ...
    public function clearCache(array $paths = [], array $options = [])
    {
        ...
        if ($clearObjects = $this->getOption('objects', $options)) {
            $objectOptions = array_merge($options, ['cache_prefix' => $this->getOption('cache_db_prefix', $options, xPDOCacheManager::CACHE_DIR)]);
        ...
        foreach ($paths as $pathIdx => $path) {
            ...
            if (file_exists($abspath)) {
                if (is_dir($abspath)) {
                    $deleted = $this->deleteTree($abspath, ['deleteTop' => false, 'skipDirs' => false, 'extensions' => $extensions]);
                } else {
                    if (unlink($abspath)) {
                        $deleted = [$path];
                    }
