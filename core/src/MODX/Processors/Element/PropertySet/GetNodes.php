...
class GetNodes extends modObjectProcessor
{
    ...
    public function getPropertySetNode()
    {
        ...
        $classes = [
            'modChunk' => $this->modx->lexicon('chunk', [], 'en'),
            'modPlugin' => $this->modx->lexicon('plugin', [], 'en'),
            'modSnippet' => $this->modx->lexicon('snippet', [], 'en'),
            'modTemplate' => $this->modx->lexicon('template', [], 'en'),
            'modTemplateVar' => $this->modx->lexicon('tv', [], 'en'),
        ];
        ...
        foreach ($classes as $class => $alias) {
            ...
            foreach ($els as $el) {
                ...
                $setArray = [
                    ...
                    'href' => '',
                    'pk' => $el->get('id'),
                    'qtip' => "<i>{$alias}</i>: <b>{$el->get('name')}</b>" . ($el->get('description') != ''
                            ? ' - ' . $el->get('description') : ''),
