...
abstract class Update extends modObjectUpdateProcessor
{
    ...
    public function beforeSave()
    {
        ...
        if (empty($name)) {
            ...
            $this->modx->error->addField($nameField, $this->modx->lexicon($this->objectType . '_err_ae', ['name' => $name]));
        }
        ...
        if (!empty($category)) {
            $category = $this->modx->getObject('modCategory', ['id' => $category]);
            if (empty($category)) {
    ...
    {
        return $this->success('', array_merge($this->object->get(['id', 'name', 'description', 'locked', 'category', 'content']), ['previous_category' => $this->previousCategory]));
    }
}
