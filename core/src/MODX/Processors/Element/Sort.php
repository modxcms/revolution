 ...
 *
 * @param string $data The JSON encoded data from the tree
 *
 * @package modx
    ...
    public function handleCategoryDrop(array $data)
    {
        ...
        foreach ($cdata as $categoryArray) {
            ...
            if ($category && $categoryArray['parent'] != $category->get('parent')) {
                $exists = $this->modx->getCount('modCategory', ['category' => $category->get('category'), 'parent' => $categoryArray['parent']]) > 0;
