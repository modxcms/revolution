...
class GetNodes extends modProcessor
{
    ...
    public function prepare()
    {
        $this->permissions = [
            ...
            'new_context_static_resource' => $this->modx->hasPermission('new_static_resource')
                ? 'pnewdoc pnew_modStaticResource' : '',
    ...
    public function getContextQuery()
    {
        ...
        if ($this->modx->getOption('context_tree_sort', null, false)) {
            ...
            $c->sortby($this->modx->getSelectColumns('modContext', 'modContext', '', [$ctxSortBy]), $ctxSortDir);
        }
    ...
    public function prepareResourceNode(modResource $resource)
    {
        ...
        $tplIcon = !empty($resource->Template) ? $resource->Template->icon : '';
        ...
        $sessionEnabled = '';
        if ($ctxSetting = $this->modx->getObject('modContextSetting', ['context_key' => $resource->get('context_key'), 'key' => 'session_enabled'])) {
        ...
        $itemArray = [
            ...
            'qtip' => $qtip,
            'preview_url' => (!$resource->get('deleted'))
                ? $this->modx->makeUrl($resource->get('id'), $resource->get('context_key'), $sessionEnabled, 'full', ['xhtml_urls' => false])
                : '',
            'page' => empty($noHref) ? '?a=' . (!empty($this->permissions['edit_document']) ? 'resource/update'
