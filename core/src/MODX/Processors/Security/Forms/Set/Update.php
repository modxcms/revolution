...
class Update extends modObjectUpdateProcessor
{
    ...
    {
        $oldRules = $this->modx->getCollection('modActionDom', [
    ...
    public function setTabRules()
    {
        ...
        if ($tabs == null) return;
        $tabs = is_array($tabs) ? $tabs : json_decode($tabs, true);
        ...
        foreach ($tabs as $tab) {
            $tabField = $this->modx->getObject('modActionField', [
                'action' => $action,
                'name' => $tab['name'],
                'type' => 'tab',
            ]);
    ...
    public function setTVRules()
    {
        ...
        if ($tvs == null) return;
        $tvs = is_array($tvs) ? $tvs : json_decode($tvs, true);
