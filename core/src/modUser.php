 ...
 * @property array                         $remote_data   Used for storing remote data for authentication for a User
 ...
 * @property modActiveUser[]               $ActiveUsers
 * @property modDashboardWidgetPlacement[] $DashboardWidgets
 *
...
class modUser extends modPrincipal
{
    ...
    public function set($k, $v = null, $vType = '')
    {
        ...
        if (in_array($k, ['password', 'cachepwd']) && $this->xpdo->getService('hashing', 'hashing.modHashing')) {
            if (!$this->get('salt')) {
    ...
    {
        $isNew = $this->isNew();
        if ($isNew && ($this->get('createdon') < 1)) {
            $this->set('createdon', time());
        }

        if ($this->xpdo instanceof modX) {
            $this->xpdo->invokeEvent('OnUserBeforeSave', [
        ...
        if ($saved && $this->xpdo instanceof modX) {
            $this->xpdo->invokeEvent('OnUserSave', [
                'mode' => $isNew ? modSystemEvent::MODE_NEW : modSystemEvent::MODE_UPD,
                'user' => &$this,
                'cacheFlag' => $cacheFlag,
            ]);
    ...
    {
        if ($this->xpdo instanceof modX) {
            $this->xpdo->invokeEvent('OnUserBeforeRemove', [
                'user' => &$this,
                'ancestors' => $ancestors,
            ]);
        }
        ...
        if ($this->xpdo instanceof modX) {
            $this->xpdo->invokeEvent('OnUserRemove', [
                'user' => &$this,
                'ancestors' => $ancestors,
            ]);
        }
    ...
    public function loadAttributes($target, $context = '', $reload = false)
    {
        ...
        if ($this->get('id') && !$reload) {
            ...
            $staleContexts = !empty($staleContexts) ? $staleContexts : [];
            $stale = array_search($context, $staleContexts);
            if ($stale !== false) {
                $reload = true;
                $staleContexts = array_diff($staleContexts, [$context]);
        ...
        if ($this->get('cachepwd')) {
            if ($this->xpdo->getService('registry',
                ...
                if ($this->xpdo->registry->user->connect()) {
                    ...
                    $msgs = $this->xpdo->registry->user->read(['poll_limit' => 1]);
                    if (!empty($msgs)) {
                        if ($key === reset($msgs)) {
    ...
    public function changePassword($newPassword, $oldPassword, $validateOldPassword = true)
    {
        ...
        if ($changePassword) {
            if (!empty ($newPassword)) {
                ...
                $changed = $this->save();
                if ($changed) {
                    $this->xpdo->invokeEvent('OnUserChangePassword', [
                        'user' => &$this,
    ...
    public function addSessionContext($context)
    {
        if (!$this->xpdo->startSession()) {
            $this->xpdo->log(xPDO::LOG_LEVEL_ERROR, "Attempt to start a session failed", '', __METHOD__, __FILE__,
                __LINE__);
    ...
    public function hasSessionContext($context)
    {
        ...
        if ($this->getSessionContexts()) {
            ...
            $hasContext = (count(array_intersect_assoc($contextTokens,
                    $this->sessionContexts)) == count($contextTokens));
        }
    ...
    public function countMessages($read = '')
    {
        ...
        $criteria = ['recipient' => $this->get('id')];
        if ($read) {
        ...
        if (empty($member)) {
            ...
            if (!$joined) {
                $this->xpdo->log(xPDO::LOG_LEVEL_ERROR,
                    'An unknown error occurred preventing adding the User to the User Group.');
            } else {
    ...
    public function sendEmail($message, array $options = [])
    {
        ...
        $mail->set(modMail::MAIL_FROM_NAME,
            $this->xpdo->getOption('fromName', $options, $this->xpdo->getOption('site_name')));
