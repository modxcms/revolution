<?php
/**
 * System Info English lexicon topic
 *
 * @language en
 * @package modx
 * @subpackage lexicon
 */
$_lang['database_charset'] = 'Кодировка база данни';
$_lang['database_name'] = 'Има на базата данни';
$_lang['database_server'] = 'Сървър база данни';
$_lang['database_tables'] = 'Таблици с База данни';
$_lang['database_optimize'] = 'Оптимизиране на база данни';
$_lang['database_table_clickhere'] = 'Кликни тук';
$_lang['database_table_clickbackup'] = 'за архивиране &amp; изтеглете избраните таблици';
$_lang['database_table_datasize'] = 'Размер на данни';
$_lang['database_table_droptablestatements'] = 'Генериране на DROP TABLE отчети.';
$_lang['database_table_effectivesize'] = 'Ефективен размер';
$_lang['database_table_indexsize'] = 'Размер на индекса';
$_lang['database_table_overhead'] = 'Превишаване';
$_lang['database_table_reserved'] = 'Резервирано';//sqlsrv
$_lang['database_table_records'] = 'Записи';
$_lang['database_table_tablename'] = 'Име на таблица';
$_lang['database_table_totalsize'] = 'Общ размер';
$_lang['database_table_totals'] = 'Общо:';
$_lang['database_table_unused'] = 'Неизползван';//sqlsrv
$_lang['database_type'] = 'Тип база данни';
$_lang['database_version'] = 'Версия база данни';
$_lang['extjs_version'] = '<a href="http://extjs.com/" target="_blank">ExtJS</a> Версия';
$_lang['localtime'] = 'Местно време';
$_lang['magpie_version'] = '<a href="http://magpierss.sourceforge.net/" target="_blank">MagpieRSS</a> версия';
$_lang['modx_version'] = 'MODX версия';
$_lang['onlineusers_action'] = 'Действие';
$_lang['onlineusers_actionid'] = 'ID действие';
$_lang['onlineusers_ipaddress'] = 'Потребителски IP адрес';
$_lang['onlineusers_lasthit'] = 'Последно действие';
$_lang['onlineusers_message'] = 'Този лист показва всички активни потребители през последните 20 минути (текущото време е ';
$_lang['onlineusers_title'] = 'Онлайн потребители';
$_lang['onlineusers_user'] = 'Потребител';
$_lang['onlineusers_userid'] = 'ID на потребителя';
$_lang['optimize_table'] = 'Кликни тук за оптимизиране на тази таблица';
$_lang['optimize_table_err'] = 'Грешка при оптимизиране на таблицата';
$_lang['phpmailer_version'] = '<a href="https://github.com/PHPMailer/PHPMailer" target="_blank">PHPMailer</a> версия';
$_lang['server'] = 'Сървър';
$_lang['servertime'] = 'Сървърно време';
$_lang['serveroffset'] = 'Сървър офсет';
$_lang['smarty_version'] = '<a href="http://smarty.net/" target="_blank">Smarty</a> версия';
$_lang['sysinfo_desc'] = 'Тук може да намерите обща информация за вашата MODX среда.';
$_lang['view_sysinfo'] = 'Системна информация';
$_lang['table_prefix'] = 'Префикс таблица';
$_lang['truncate_table'] = 'Кликнете тук за да изчистите таблицата';
$_lang['truncate_table_err'] = 'Грешка при изчистване на таблицата';
$_lang['version_codename'] = 'Кодово име на версията';