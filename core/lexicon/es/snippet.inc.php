<?php
/**
 * Snippet Spanish lexicon topic
 *
 * @language es_ES
 * @package MODX
 * @subpackage lexicon
 *
 * @author libnova, S.L. <http://www.libnova.com/>
 * @updated 2014-03-10
 */
$_lang['snippet'] = 'Snippet';
$_lang['snippets_available'] = 'Snippets disponibles';
$_lang['snippet_code'] = 'Código del Snippet (php)';
$_lang['snippet_delete_confirm'] = '¿Estás seguro de que quieres eliminar este snippet?';
$_lang['snippet_desc'] = 'Descripción';
$_lang['snippet_desc_category'] = 'La Categoría a la que pertenece el Snippet.';
$_lang['snippet_desc_description'] = 'Breve descripción del Snippet.';
$_lang['snippet_desc_name'] = 'El nombre del Snippet. Estará disponible usando la etiqueta [[NameOfSnippet]].';
$_lang['snippet_duplicate_confirm'] = '¿Estás seguro de que quieres duplicar este snippet?';
$_lang['snippet_duplicate_error'] = 'Ocurrió un error mientras se duplicaba el snippet.';
$_lang['snippet_err_create'] = 'Ocurrió un error mientras se creaba el snippet.';
$_lang['snippet_err_delete'] = 'Ocurrió un error mientras se trataba de eliminar el snippet.';
$_lang['snippet_err_duplicate'] = 'Ocurrió un error mientras se trataba de duplicar el snippet.';
$_lang['snippet_err_exists_name'] = 'Ya existe un snippet con el nombre "[[+name]]".';
$_lang['snippet_err_invalid_name'] = 'El nombre del Snippet no es válido.';
$_lang['snippet_err_locked'] = 'La edición de este snippet está bloqueada.';
$_lang['snippet_err_nf'] = '¡Snippet no encontrado!';
$_lang['snippet_err_ns'] = '¡Snippet no especificado!';
$_lang['snippet_err_ns_name'] = 'Por favor, especifica un nombre para el snippet.';
$_lang['snippet_err_remove'] = 'Ocurrió un error mientras se eliminaba el snippet.';
$_lang['snippet_err_save'] = 'Ocurrió un error mientras se guardaba el snippet.';
$_lang['snippet_execonsave'] = 'Ejecutar snippet despues de guardarlo.';
$_lang['snippet_lock'] = 'Bloquear edición del snippet';
$_lang['snippet_lock_msg'] = 'Los usuarios deben poseer el permiso "edit_locked" para poder editar este snippet.';
$_lang['snippet_management_msg'] = 'Aquí se puede escoger el snippet a editar.';
$_lang['snippet_msg'] = 'Aquí se pueden añadir/editar snippets. Recuerda que los snippets son código PHP, y para que la salida del snippet sea mostrada en la plantilla, se necesita devolver un valor desde dentro del snippet.';
$_lang['snippet_name'] = 'Nombre del Snippet';
$_lang['snippet_new'] = 'Nuevo Snippet';
$_lang['snippet_properties'] = 'Propiedades del Snippet';
$_lang['snippet_title'] = 'Crear/editar snippet';
$_lang['snippet_untitled'] = 'Snippet sin nombre';
$_lang['snippets'] = 'Snippets';
