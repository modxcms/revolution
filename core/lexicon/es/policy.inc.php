<?php
/**
 * Access Policy Spanish lexicon topic
 *
 * @language es_ES
 * @package MODX
 * @subpackage lexicon
 *
 * @author libnova, S.L. <http://www.libnova.com/>
 * @updated 2014-03-10
 */
$_lang['active_of'] = '[[+active]] d [[+total]]';
$_lang['active_permissions'] = 'Permisos Activos';
$_lang['no_policy_option'] = '(sin política) ';
$_lang['permission'] = 'Permiso';
$_lang['permission_add'] = 'Añadir Permiso';
$_lang['permission_add_template'] = 'Añadir Permiso a Plantilla';
$_lang['permission_err_ae'] = 'El permiso ya existe para esta política.';
$_lang['permission_err_nf'] = 'Permiso no encontrado.';
$_lang['permission_err_ns'] = 'Permiso no especificado.';
$_lang['permission_err_remove'] = 'Ocurrió un error mientras se trataba de eliminar este permiso.';
$_lang['permission_err_save'] = 'Ocurrió un error mientra se trataba de guardar este permiso.';
$_lang['permission_new'] = 'Permiso Nuevo';
$_lang['permission_remove'] = 'Eliminar Permiso';
$_lang['permission_remove_confirm'] = '¿Estás seguro de que quieres eliminar este permiso?';
$_lang['permission_update'] = 'Actualizar Permiso';
$_lang['permissions'] = 'Permisos';
$_lang['permissions_desc'] = 'Aquí puedes definir los permisos específicos que contiene esta política. Todos lo grupos de usuarios con esta política heredarán estos permisos.';
$_lang['policies'] = 'Políticas de Acceso';
$_lang['policy'] = 'Política de Acceso';
$_lang['policy_create'] = 'Crear Política de Acceso';
$_lang['policy_data'] = 'Datos de Política';
$_lang['policy_desc'] = 'Las políticas de acceso son políticas genéricas que restringen o habilitan ciertas acciones dentro de MODX.';
$_lang['policy_desc_name'] = 'El nombre de la política de Acceso';
$_lang['policy_desc_description'] = 'Opcional. Breve descripción de la política de acceso';
$_lang['policy_desc_template'] = 'La plantilla de política utilizada para esta política. Las políticas toman su lista de permisos de su plantilla..';
$_lang['policy_desc_lexicon'] = 'Opcional. El Ámbito de Archivo de Idioma que utilizará esta Política para traducir los permisos que posee.';
$_lang['policy_duplicate'] = 'Duplicar política';
$_lang['policy_duplicate_confirm'] = '¿Estás seguro de que quieres duplicar esta política y todos sus datos?';
$_lang['policy_err_ae'] = 'Ya existe una política con el nombre `[[+name]]`. Por favor, selecciona otro nombre.';
$_lang['policy_err_nf'] = 'Política no encontrada.';
$_lang['policy_err_ns'] = 'Política no especificada.';
$_lang['policy_err_remove'] = 'Ocurrió un error mientras se trataba de eliminar la política.';
$_lang['policy_err_save'] = 'Ocurrió un error mientras se trataba de guardar la política.';
$_lang['policy_export'] = 'Exportar política';
$_lang['policy_import'] = 'Importar política';
$_lang['policy_import_msg'] = 'Selecciona un archivo XML desde el que importar una Política. Debe ser un archivo de Política en su correspondiente formato XML.';
$_lang['policy_management'] = 'Políticas de Acceso';
$_lang['policy_management_msg'] = 'Las Políticas de Acceso administran cómo MODX maneja los permisos para las acciones especificadas.';
$_lang['policy_name'] = 'Nombre de la Política';
$_lang['policy_property_create'] = 'Crear Propiedad de Política de Acceso';
$_lang['policy_property_new'] = 'Propiedad de Política Nueva';
$_lang['policy_property_remove'] = 'Eliminar Propiedad de Política Nueva';
$_lang['policy_property_specify_name'] = 'Por favor especifíca un nombre de propiedad de política:';
$_lang['policy_remove'] = 'Eliminar Política';
$_lang['policy_remove_confirm'] = '¿Estás seguro de que quieres eliminar esta política de Acceso?';
$_lang['policy_remove_multiple'] = 'Eliminar Políticas';
$_lang['policy_remove_multiple_confirm'] = '¿Estás seguro de que quieres eliminar estas políticas de Acceso? Esto es irreversible.';
$_lang['policy_update'] = 'Actualizar política';
$_lang['policy_template'] = 'Plantilla de Política';
$_lang['policy_template_create'] = 'Crear Plantilla de Política';
$_lang['policy_template.desc'] = 'Una Plantilla de Política define qué Permisos serán mostrados en la cuadrícula de Permisos cuando se esté editando una política específica. Puedes añadir o eliminar Permisos específicos de esta plantilla de abajo. Nota: Al eliminar un Permiso de una Plantilla, lo eliminará de cualquier política que use dicha Plantilla.';
$_lang['policy_template_desc_name'] = 'El nombre de la Plantilla de Política de Acceso';
$_lang['policy_template_desc_description'] = 'Opcional. Breve descripción de la Plantilla de Política de Acceso';
$_lang['policy_template_desc_lexicon'] = 'Opcional. El Ambito de Archivo de Idioma que utilizará esta Plantilla de Política para traducir los permisos que posee.';
$_lang['policy_template_desc_template_group'] = 'El Grupo de Plantillas de Política a utilizar. Ésto se utiliza cuando se seleccionan políticas en un menú desplegable; se suelen filtrar por el Grupo de Plantillas. Selecciona un grupo apropiado para tu Plantilla de Política.';
$_lang['policy_template_duplicate'] = 'Duplicar Plantilla de Política';
$_lang['policy_template_duplicate_confirm'] = '¿Estás seguro de que quieres duplicar esta Plantilla de política?';
$_lang['policy_template_err_ae'] = 'Ya existe una Plantilla de política con el nombre `[[+name]]`. Por favor, selecciona otro nombre.';
$_lang['policy_template_err_nf'] = 'Plantilla de Política no encontrada.';
$_lang['policy_template_err_ns'] = 'Plantilla de Política no especificada.';
$_lang['policy_template_err_remove'] = 'Ocurrió un error mientras se trataba de eliminar la Plantilla de Política.';
$_lang['policy_template_err_save'] = 'Ocurrió un error mientras se trataba de guardar la Plantilla de Política.';
$_lang['policy_template_export'] = 'Exportar Plantilla de Política';
$_lang['policy_template_import'] = 'Importar Plantilla de Política';
$_lang['policy_template_import_msg'] = 'Selecciona un archivo XML desde el que importar una Plantilla de Política. Debe ser un archivo de Plantilla de Política en su correspondiente formato XML.';$_lang['policy_template_create'] = 'Crear Plantilla de Política';
$_lang['policy_template_remove'] = 'Eliminar Plantilla de política';
$_lang['policy_template_remove_confirm'] = '¿Estás seguro de que quieres eliminar esta Plantilla de Política? También eliminará todas las políticas adjuntas a esta Plantilla - esto podría hacer que tu instalación de MODX no vuelva a funcionar si cualquier política activa está adjunta a esta Plantilla.';
$_lang['policy_template_remove_multiple'] = 'Eliminar Plantillas de Política Seleccionadas';
$_lang['policy_template_remove_multiple_confirm'] = '¿Estás seguro de que quieres eliminar estas Plantillas de Política? También eliminará todas las políticas adjuntas a estas Plantillas - esto podría hacer que tu instalación de MODX no vuelva a funciona si cualquiera de las políticas activas están adjuntas a estas Plantillas.';
$_lang['policy_template_update'] = 'Actualizar Plantilla de Política';
$_lang['policy_templates'] = 'Plantillas de Política';
$_lang['policy_templates.intro_msg'] = 'Esta es una lista de las Plantillas de Política, las cuales definen listas de los Permisos que están seleccionados o no en políticas específicas.';
$_lang['template_group'] = 'Grupo de Plantillas';
