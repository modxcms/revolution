<?php
/**
 * About English lexicon topic
 *
 * @language en
 * @package modx
 * @subpackage lexicon
 */
$_lang['help_page_title'] = 'Help & Support';
$_lang['help_main_head'] = 'MODX è qui per voi';
$_lang['help_main_subhead'] = 'Leggete sotto per ottenere l\'aiuto necessario per il vostro progetto MODX <br>Potete avere maggiori dettagli su <a href="http://modx.com/support"> modx.com/support</a> o chiamando il + 1 (469) 777-6639';
$_lang['forums'] = 'Forums';
$_lang['forums_title'] = 'MODX Community Forums';
$_lang['forums_description'] = 'Aiuto gratuito da parte della Community';
$_lang['forums_link'] = 'http://forums.modx.com/';
$_lang['docs'] = 'Documentazione';
$_lang['docs_title'] = 'Leggi il Fantastico Manuale (di MODX)';
$_lang['docs_description'] = 'Leggi i manuali ufficiali di MODX';
$_lang['docs_link'] = 'http://rtfm.modx.com/';
$_lang['bugs'] = 'Bug Tracker';
$_lang['bugs_title'] = 'Segnalazione Bugs & Richiesta di nuove funzionalità';
$_lang['bugs_description'] = 'Hai trovato un problema? Faccelo sapere!';
$_lang['bugs_link'] = 'https://github.com/modxcms/revolution/issues';
$_lang['support'] = 'Supporto Commerciale';
$_lang['support_title'] = 'Acquista ora un Ticket di supporto d\'emergenza';
$_lang['support_description'] = 'Supporto prioritario all\'origine';
$_lang['support_link'] = 'http://modx.com/modx-support/';
$_lang['email_placeholder'] = 'you@example.com';
$_lang['email_sub'] = 'Rimani aggiornato con MODX';
$_lang['email_sub_description'] = 'Iscriviti alla <b>Newsletter di MODX</b> per ricevere tutte le notizie importanti su MODX. Scrivi la tua email qui sotto, vi promettiamo di non inviare mai spam.';
$_lang['email_sub_button'] = 'Registrati';
$_lang['social_follows'] = 'È inoltre possibile <b>seguire MODX</b> su questi canali.';
$_lang['follow'] = 'Follow MODX';
$_lang['like'] = 'Mi Piace MODX';
$_lang['circle'] = 'Aggiungi MODX alle tue cerchie';
$_lang['help_about'] = 'Il software MODX ® che stai utilizzando è il risultato della collaborazione con un\'incredibile comunità di utenti, sostenitori e sviluppatori. Dal 2004, la squadra dietro MODX lo ha sponsorizzato e gestito, finanziando i lavori in corso attraverso il supporto commerciale e le richieste commissionate. È possibile <a href="http://modx.com/support/" class="supportTicket"> acquistare il supporto all\'origine</a>, per ottenere un\'ora del supporto di emergenza (utilizzabile anche per due ore di supporto non-prioritario). O <a href="mailto:hello@modx.com?subject=MODX Manager Inquiry"> scriveteci</a> se avete un\'altra domanda su MODX in generale.';
$_lang['help_credit'] = 'MODX è un software libero e open source concesso sotto la <a href="http://www.gnu.org/licenses/gpl-2.0.html" target="_blank"> licenza GPL versione 2.0</a> o successiva. Copyright 2004-2015 di MODX, LLC. "MODX" è un marchio registrato. Fai la cosa giusta — per favore mantieni questi credits e le altre informazioni su questa pagina intatte.';
