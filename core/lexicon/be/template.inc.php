<?php
/**
 * Template English lexicon topic
 *
 * @language en
 * @package modx
 * @subpackage lexicon
 */

$_lang['access'] = 'Доступ';
$_lang['filter_by_category'] = 'Фільтраваць па катэгорыi…';
$_lang['rank'] = 'Ранг';
$_lang['template'] = 'Шаблон';
$_lang['template_assignedtv_tab'] = 'Прызначаныя зменныя шаблону';
$_lang['template_code'] = 'Код шаблону (HTML)';
$_lang['template_desc'] = 'Апісанне';
$_lang['template_desc_category'] = 'Катэгорыя, да якой належыць шаблон.';
$_lang['template_desc_description'] = 'Кароткае апісанне гэтага шаблону.';
$_lang['template_desc_name'] = 'Назва гэтага шаблону.';
$_lang['template_delete_confirm'] = 'Вы сапраўды жадаеце выдаліць гэты шаблон?';
$_lang['template_duplicate_confirm'] = 'Вы сапраўды жадаеце дубляваць гэты шаблон?';
$_lang['template_edit_tab'] = 'Рэдагаваць шаблон';
$_lang['template_empty'] = '(пуста)';
$_lang['template_err_default_template'] = 'Гэты шаблон зададзены як шаблон па змаўчанні. Калі ласка, выберыце іншы шаблон па змаўчанні ў канфігурацыі MODX перад выдаленнем гэтага шаблону.<br />';
$_lang['template_err_delete'] = 'Адбылася памылка пры выдаленні шаблону.';
$_lang['template_err_duplicate'] = 'Адбылася памылка пры дубляванні шаблону.';
$_lang['template_err_ae'] = 'Шаблон ужо існуе з назвай "[[+name]]".';
$_lang['template_err_in_use'] = 'Гэты шаблон выкарыстоўваецца. Калі ласка, змяніце шаблон на другі для дакументаў, якія выкарыстоўваюць гэты шаблон. Спіс дакументаў, якія выкарыстоўваюць шаблон: <br />';
$_lang['template_err_locked'] = 'Шаблон заблакаваны для рэдагавання.';
$_lang['template_err_nf'] = 'Шаблон не знойдзены!';
$_lang['template_err_ns'] = 'Шаблон не пазначаны.';
$_lang['template_err_ns_name'] = 'Калі ласка, пазначце назву для шаблону.';
$_lang['template_err_remove'] = 'Адбылася памылка пры выдаленні шаблону.';
$_lang['template_err_save'] = 'Адбылася памылка пры захаванні шаблону.';
$_lang['template_icon'] = 'Значок';
$_lang['template_icon_description'] = 'Неабавязкова. Карыстальніцкі CSS-клас значка для ўсіх рэсурсаў з гэтым шаблонам. Напрыклад, класс ад Font Awesome (бясплатна), такі як icon-home альбо fa-home.';
$_lang['template_lock'] = 'Заблакаваць шаблон для рэдагавання';
$_lang['template_lock_msg'] = 'Карыстальнікі павінны мець дазвол edit_locked для рэдагавання гэтага шаблону.';
$_lang['template_locked_message'] = 'Гэты шаблон заблакаваны.';
$_lang['template_management_msg'] = 'Тут вы можаце выбраць які шаблон вы жадаеце рэдагаваць.';
$_lang['template_msg'] = 'Стварэнне і рэдагаванне шаблонаў. Змененыя або новыя шаблоны не будуць бачны на кэшаваных старонках сайту пакуль кэш не будзе ачышчаны; аднак, вы можаце выкарыстоўваць функцыю папярэдняга прагляду на старонцы каб убачыць шаблон у дзеянні.';
$_lang['template_name'] = 'Назва шаблону';
$_lang['template_new'] = 'Новы шаблон';
$_lang['template_no_tv'] = 'Яшчэ ніводнай зменнай шаблону не прызначана да гэтага шаблону.';
$_lang['template_properties'] = 'Параметры па змаўчанні';
$_lang['template_reset_all'] = 'Скінуць усе старонкі да выкарыстання шаблону па змаўчанні';
$_lang['template_reset_specific'] = 'Скінуць только \'%s\' старонак';
$_lang['template_title'] = 'Стварыць/рэдагаваць шаблон';
$_lang['template_tv_edit'] = 'Рэдагваць парадак сартавання зменных шаблону';
$_lang['template_tv_msg'] = 'Зменныя шаблону прызначаныя да гэтага шаблону пералічаны ніжэй.';
$_lang['template_untitled'] = 'Шаблон без назвы';
$_lang['templates'] = 'Шаблоны';
$_lang['tvt_err_nf'] = 'Зменная шаблону не мае доступу да пазначанага шаблону.';
$_lang['tvt_err_remove'] = 'Адбылася памылка пры спробе выдаліць зменную шаблону з шаблону.';
