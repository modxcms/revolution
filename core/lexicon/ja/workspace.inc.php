...
$_lang['packages_desc'] = '「パッケージ」はMODXを構成するコンポーネント(拡張機能やテンプレート・管理画面デザイン・サンプルデータなど、コア以外の拡張要素)を、マウス操作だけで手軽にインストール及び更新・削除できるようにまとめたものです。パッケージの配布元(パッケージプロバイダ)を選択することもでき、デフォルトではMODX本家開発サイトのエクストラ配布エリアがセットされています。パッケージ管理の詳細については、各グリッドの右クリックで表示されるコンテキストメニューを確認してください。<br />パッケージをプロバイダから選択したい場合は<b>「プロバイダから取得」</b>を利用してください。<br />/core/packages/ ディレクトリに手作業で置いたパッケージを管理対象として追加したい場合、またはプロバイダを選択したい場合は<b>「取得ウィザード」</b>を利用してください。';
