<?php
/**
 * Snippet English lexicon topic
 *
 * @language en
 * @package modx
 * @subpackage lexicon
 */
$_lang['example_tag_snippet_name'] = 'SnippetNaam';
$_lang['snippet'] = 'Snippet';
$_lang['snippets_available'] = 'Snippets beschikbaar voor jou om te includen in jouw pagina';
$_lang['snippet_category_desc'] = 'Gebruik om snippets te groeperen in de Elementenboom.';
$_lang['snippet_code'] = 'Snippet Code (PHP)';
$_lang['snippet_delete_confirm'] = 'Weet je zeker dat je deze snippet wilt verwijderen?';
$_lang['snippet_description_desc'] = 'Gebruiksinformatie voor deze snippet die wordt weergegeven in zoekresultaten en als een tooltip in de Elementenboom.';
$_lang['snippet_duplicate_confirm'] = 'Weet je zeker dat je deze snippet wilt dupliceren?';
$_lang['snippet_duplicate_error'] = 'Er is een fout opgetreden tijdens het dupliceren van de snippet.';
$_lang['snippet_err_create'] = 'Er is een fout opgetreden tijdens het maken van de snippet.';
$_lang['snippet_err_delete'] = 'An error occurred while trying to delete the snippet.';
$_lang['snippet_err_duplicate'] = 'An error occurred while trying to duplicate the snippet.';
$_lang['snippet_err_ae'] = 'Een snippet met deze naam "[[+name]]" bestaat reeds.';
$_lang['snippet_err_invalid_name'] = 'Snippetnaam is ongeldig.';
$_lang['snippet_err_locked'] = 'Deze snippet is vergrendeld voor bewerken.';
$_lang['snippet_err_nf'] = 'Snippet niet gevonden!';
$_lang['snippet_err_ns'] = 'Snippet niet gespecificeerd.';
$_lang['snippet_err_ns_name'] = 'Vul een naam in voor de snippet.';
$_lang['snippet_err_remove'] = 'Er is een fout opgetreden tijdens het verwijderen van de snippet.';
$_lang['snippet_err_save'] = 'Er is een fout opgetreden tijdens het opslaan van de snippet.';
$_lang['snippet_execonsave'] = 'Snippet uitvoeren na opslaan.';
$_lang['snippet_lock'] = 'Vergrendel snippet voor bewerken';
$_lang['snippet_lock_desc'] = 'Alleen gebruikers met rechten "edit_locked" kunnen deze snippet bewerken.';
$_lang['snippet_management_msg'] = 'Hier kun je kiezen welke snippet je wilt bewerken.';
$_lang['snippet_name_desc'] = 'Plaats de inhoud gegenereerd door deze snippet in een document, sjabloon of chunk met behulp van de volgende MODX tag: [[+tag]]';
$_lang['snippet_new'] = 'Maak snippet';
$_lang['snippet_properties'] = 'Standaard eigenschappen';
$_lang['snippet_tab_general_desc'] = 'Hier kunt u de basisattributen voor deze <em>Snippet</em> en de inhoud ervan invullen. De inhoud moet PHP zijn, ofwel geplaatst in het <em>Snippet Code</em> veld hieronder of in een statisch extern bestand. Om uitvoer van je snippet te ontvangen op de plaats waar deze wordt genoemd (binnen een sjabloon of chunk), moet een waarde worden geretourneerd vanuit de code.';
$_lang['snippet_tag_copied'] = 'Snippet tag gekopieerd!';
$_lang['snippets'] = 'Snippets';

// Temporarily match old keys to new ones to ensure compatibility
// --fields
$_lang['snippet_desc_category'] = $_lang['snippet_category_desc'];
$_lang['snippet_desc_description'] = $_lang['snippet_description_desc'];
$_lang['snippet_desc_name'] = $_lang['snippet_name_desc'];
$_lang['snippet_lock_msg'] = $_lang['snippet_lock_desc'];

// --tabs
$_lang['snippet_msg'] = $_lang['snippet_tab_general_desc'];
