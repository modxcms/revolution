...
$_lang['configcheck_htaccess_msg'] = 'MODX обнаружил, что папка ядра (частично) публично доступна.
<strong>Это не рекомендуется и опасно для безопасности.</strong>
Если ваша установка MODX запущена на веб-сервере Apache,
вы должны создать . htaccess file внутри папки ядра <em>[[+fileLocation]]</em>.
Это также можно сделать, переименовав существующий уже файл-пример ht.access в .htaccess.
<p>Также существуют и другие методы и веб-серверы, которые вы можете использовать, прочтите <a href="https://docs.modx.com/3.x/en/getting-started/maintenance/securing-modx">Руководство по усилению защиты MODX</a>
для дополнительной информации о защите вашего сайта.</p>
Если вы правильно настроили все, откройте в браузере, например, к <a href="[[+checkUrl]]" target="_blank">Changelog</a>
эта ссылка должна дать вам 403 код ответа (разрешение запрещено) или 404 (не найдено). Если вы можете увидеть список изменений
в браузере, что-то все еще не так и вам нужно перенастроить конфигурацию или найти специалиста для решения этой проблемы.';
