<?php
$_lang['element'] = 'Элемент';
$_lang['element_err_nf'] = 'Элемент не найден.';
$_lang['element_err_ns'] = 'Элемент не указан.';
$_lang['is_static'] = 'Статичный';
$_lang['is_static_msg'] = 'Указывает, что содержимое элемента хранятся во внешнем файле.';
$_lang['quick_create'] = 'Быстро создать';
$_lang['quick_create_chunk'] = 'Быстро создать чанк';
$_lang['quick_create_plugin'] = 'Быстро создать плагин';
$_lang['quick_create_snippet'] = 'Быстро создать сниппет';
$_lang['quick_create_template'] = 'Быстро создать шаблон';
$_lang['quick_create_tv'] = 'Быстро создать TV';
$_lang['quick_update_chunk'] = 'Быстро обновить чанк';
$_lang['quick_update_plugin'] = 'Быстро обновить плагин';
$_lang['quick_update_snippet'] = 'Быстро обновить сниппет';
$_lang['quick_update_template'] = 'Быстро обновить шаблон';
$_lang['quick_update_tv'] = 'Быстро обновить TV';
$_lang['properties_export'] = 'Экспорт параметров';
$_lang['properties_import'] = 'Импорт параметров';
$_lang['static_file'] = 'Статичный файл';
$_lang['static_file_msg'] = 'Расположение файла в котором храниться содержимое ресурса';
$_lang['static_source'] = 'Источник файлов для статичного файла';
$_lang['static_source_msg'] = 'MODX will use this Media Source as a base path when looking for the Static Element file. Использование «Нет» означает, что вы указываете абсолютный путь.';
$_lang['tv_elements'] = 'Возможные значения';
$_lang['tv_default'] = 'Значение по умолчанию';
$_lang['tv_type'] = 'Тип ввода';
$_lang['tv_output_type'] = 'Тип вывода';
$_lang['tv_output_type_properties'] = 'Свойства типа вывода';