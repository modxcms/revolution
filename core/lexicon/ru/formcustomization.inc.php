<?php
/**
* Form Customization Russian lexicon topic
*
* @language ru
* @package modx
* @subpackage lexicon
*/
$_lang['action'] = 'Действие';
$_lang['action_desc'] = 'Действие, к которому будет применяться это правило';
$_lang['activate'] = 'Включить';
$_lang['constraint'] = 'Ограничивающее значение';
$_lang['constraint_class'] = 'Ограничивающий класс';
$_lang['constraint_class_desc'] = 'Необязательно. Если задано значение, то вместе с ограничивающим полем и настройками ограничения, сведёт это правило к применённым ограничениям.';
$_lang['constraint_desc'] = 'Необязательно. Значение ограничивающего поля, при наличие которого правило будет работать.';
$_lang['constraint_field'] = 'Ограничивающее поле';
$_lang['constraint_field_desc'] = 'Необязательно. Поле при наличии которого правило будет работать.';
$_lang['containing_panel'] = 'Родительская панель';
$_lang['containing_panel_desc'] = 'Идентификатор панели  в которой находится область. Необходим в некоторых правилах, для того что бы система могла понять в какой форме или панели находится область.';
$_lang['deactivate'] = 'Выключить';
$_lang['default_value'] = 'Значение по умолчанию';
$_lang['export'] = 'Экспорт';
$_lang['field'] = 'Область';
$_lang['field_desc'] = 'Это область к которой применяется правило. Это может быть поле, вкладка или дополнительное поле. Если это дополнительное поле, то указывайте его в формате: &laquo;tv#&raquo;, где # это идентификатор дополнительного поля.';
$_lang['field_default'] = 'Значение поля по умолчанию';
$_lang['field_label'] = 'Название поля';
$_lang['field_visible'] = 'Видимость поля';
$_lang['fields'] = 'Поля';
$_lang['file'] = 'Файл';
$_lang['filter_by_action'] = 'Фильтровать по действию...';
$_lang['filter_by_rule_type'] = 'Фильтровать по правилу...';
$_lang['filter_by_search'] = 'Поиск...';
$_lang['for_parent'] = 'Для контейнера';
$_lang['for_parent_desc'] = 'Отметьте это пункт, если необходимо чтобы правило применялось только к ресурсам контейнерам. Only use with Resources or objects with a `parent` field. Useful for the "create" page of Resources.';
$_lang['form_customization_msg'] = 'Здесь вы можете создавать и редактировать профили настроек форм. Чтобы открыть страницу с документацией, щёлкните по кнопке "Помощь!"  в правом верхнем углу. Дважды щелкните по строке таблицы, чтобы изменить её значения. Щёлкните правой кнопкой мыши по строке таблицы чтобы открыть контекстное меню.';
$_lang['form_rules'] = 'Правила настройки форм';
$_lang['import'] = 'Импорт';
$_lang['import_from_xml'] = 'Импортировать новый набор правил из XML';
$_lang['label'] = 'Новое имя';
$_lang['original_value'] = 'оригинальное значение';
$_lang['profile'] = 'Профиль';
$_lang['profile_create'] = 'Создать новый профиль';
$_lang['profile_err_nfs'] = 'Профиль настройки форм с  идентификатором [[+id]] не найден.';
$_lang['profile_err_ns'] = 'Профиль настройки форм не указан!';
$_lang['profile_err_remove'] = 'Произошла ошибка при попытке удалить профиль.';
$_lang['profile_err_save'] = 'Произошла ошибка при попытке сохранить профиль.';
$_lang['profile_msg'] = 'Здесь вы можете указать наборы правил для этого профиля. Наборы правил применяются к странице (создания или редактирования ресурса). Применение набора может быть ограничено конкретным шаблоном или любым другим полем ресурса. Ограничения  для страницы создания нового ресурса используют значения полей родительского ресурса. Для редактирования конкретного набора правил щёлкните по нему правой кнопкой мыши и выберите необходимый пункт меню.';
$_lang['profile_new'] = 'Новый профиль настройки форм';
$_lang['profile_remove'] = 'Удалить профиль';
$_lang['profile_remove_confirm'] = 'Вы уверены, что хотите удалить этот профиль? Это необратимо.';
$_lang['profile_remove_multiple'] = 'Удаление нескольких профилей';
$_lang['profile_remove_multiple_confirm'] = 'Вы уверены, что хотите удалить эти профили? Это необратимо.';
$_lang['profile_usergroup_err_ae'] = 'Эта группа пользователей уже назначена этому профилю!';
$_lang['profile_usergroups_msg'] = 'Здесь вы можете назначить группы пользователей, которые будут использовать этот профиль. Если группы пользователей не назначены, все пользователи будут использовать этот профиль.';
$_lang['profiles'] = 'Профили настройки форм';
$_lang['profiles_msg'] = 'Это список  профилей с наборами правил для настройки форм. Профиль может быть включен или выключен. Они также могут быть настроены для применения только к определенным группам пользователей.';
$_lang['rank'] = 'Порядок выполнения';
$_lang['rank_desc'] = 'Порядок в котором правила будут выполняться. Меньшее число означает, что правило будет выполнено раньше.';
$_lang['rule'] = 'Правило';
$_lang['rule_create'] = 'Создать правило';
$_lang['rule_desc'] = 'Тип правила которое будет применяться к области.';
$_lang['rule_description_desc'] = 'Необязательно. Описание правила.';
$_lang['rule_err_ae'] = 'Такое правило для этой области уже есть!';
$_lang['rule_err_duplicate'] = 'Произошла ошибка при попытке сделать копию правила.';
$_lang['rule_err_nf'] = 'Правило не найдено.';
$_lang['rule_err_ns'] = 'Правило не указано.';
$_lang['rule_err_remove'] = 'Произошла ошибка при попытке удалить правило.';
$_lang['rule_err_save'] = 'Произошла ошибка при попытке сохранить правило.';
$_lang['rule_remove'] = 'Удалить правило';
$_lang['rule_remove_confirm'] = 'Вы уверены, что хотите удалить это правило?';
$_lang['rule_remove_multiple'] = 'Удалить несколько правил';
$_lang['rule_remove_multiple_confirm'] = 'Вы уверены, что хотите удалить эти правила? Это необратимо.';
$_lang['rule_update'] = 'Обновить правило';
$_lang['rule_value_desc'] = 'Значение, присваиваемое правилу.';
$_lang['rules'] = 'Правила';
$_lang['set'] = 'Набор правил';
$_lang['set_and_fields'] = 'Информация о наборе правил и поля ресурса';
$_lang['set_change_template'] = 'Изменить шаблон связанный с набором правил';
$_lang['set_change_template_confirm'] = ' Изменить шаблон? Это изменит шаблон которому назначены эти правила. If so, MODx will first save your changes before reloading the page to refresh the new TVs for the new Template.';
$_lang['set_constraint_field_desc'] = 'Указывая ограничивающее поле вы ограничиваете применение этого правила ресурсами у которых значение этого поля равно значению указанному в поле "Ограничивающее значение".';
$_lang['set_constraint_desc'] = 'Укажите значение ограничивающего поля (указанного выше) для ограничения выполнения правил из этого набора.';
$_lang['set_create'] = 'Новый набор правил';
$_lang['set_err_nfs'] = 'Не найден набор с идентификатором [[+id]]';
$_lang['set_err_ns'] = 'Набор правил не указан.';
$_lang['set_fields_msg'] = 'Здесь вы можете изменить настройки стандартных полей ресурса для этого набора правил, их видимость, названия и значения по умолчанию. Просто дважды щелкните по строке таблицы, чтобы изменить её значения. Оставьте поле пустым для того чтобы использовать значение по умолчанию.';
$_lang['set_import_err_upload'] = 'Произошла ошибка при попытке найти XML файл. Please specify a valid file.';
$_lang['set_import_err_xml'] = 'An error occurred importing the XML file. Please ensure you specified a valid Form Customization Set XML File.';
$_lang['set_import_msg'] = 'Выберите XML файл для импорта из него набора правил настройки форм. It must be in the correct XML Form Customization format.';
$_lang['set_msg'] = 'Здесь вы можете настроить стандартные поля, вкладки и дополнительные поля для страницы к которой применяется этот набор правил.';
$_lang['set_new'] = 'Создать новый набор правил';
$_lang['set_remove'] = 'Удалить набор правил';
$_lang['set_remove_confirm'] = 'Вы уверены, что хотите удалить этот набор правил? Это необратимо.';
$_lang['set_remove_multiple'] = 'Удалить несколько наборов правил';
$_lang['set_remove_multiple_confirm'] = 'Вы уверены, что хотите удалить эти наборы правил? Это необратимо.';
$_lang['set_tab_err_ae'] = 'Вкладка с этим идентификатором уже есть. Пожалуйста, укажите другой идентификатор.';
$_lang['set_tabs_msg'] = 'Здесь вы можете настроить видимость и названия вкладок для этого набора правил, так-же можно создать новые вкладки.';
$_lang['set_template_desc'] = 'Выбирая шаблон вы ограничиваете выполнение правил из этого набора только ресурсами с указанным шаблоном.';
$_lang['set_tvs_msg'] = 'Здесь можно изменить настройки для дополнительных полей ресурса, их видимость, название, значение по умолчанию и указать вкладку в которой они должны отображаться. Внимание: Если вы перемещаете дополнительное поле в другую вкладку, то порядок показа  дополнительных полей можно настроить с помощью поля "Порядок сортировки" .';
$_lang['sets'] = 'Наборы правил для настройки форм';
$_lang['simplexml_err_nf'] = 'MODx requires the SimpleXML PHP extension to use this feature. Please ensure the extension is installed before continuing.';
$_lang['tab'] = 'Вкладка';
$_lang['tab_create'] = 'Создать новую вкладку';
$_lang['tab_id'] = 'ID';
$_lang['tab_name'] = 'Имя вкладки';
$_lang['tab_title'] = 'Заголовок вкладки';
$_lang['tab_new'] = 'Новая вкладка';
$_lang['tab_rank'] = 'Порядок сортировки';
$_lang['tab_remove'] = 'Удалить вкладку';
$_lang['tab_remove_confirm'] = 'Вы уверены, что хотите удалить эту вкладку?';
$_lang['tab_visible'] = 'Видимость вкладки';
$_lang['tabs'] = 'Вкладки';
$_lang['tv'] = 'Дополнительное поле(TV)';
$_lang['tv_default'] = 'Значение дополнительного поля по умолчанию';
$_lang['tv_label'] = 'Заголовок дополнительного поля';
$_lang['tv_name'] = 'Имя';
$_lang['tv_move'] = 'Переместить TV во вкладку';
$_lang['tv_visible'] = 'Видимость дополнительного поля';
$_lang['tvs'] = 'Дополнительные поля';
$_lang['usergroup'] = 'Группа пользователей';
$_lang['usergroup_create'] = 'Добавить группу пользователей';
$_lang['usergroup_desc'] = 'Не обязательно. Если указано, то правило будет применяться только к пользователям из указанной группы.';
$_lang['usergroup_remove'] = 'Удалить группу пользователей из набора';
$_lang['usergroup_remove_confirm'] = 'Вы уверены, что хотите чтобы этот набор правил больше не был связан с этой группой пользователей?';
$_lang['usergroups'] = 'Группы пользователей';
$_lang['visible'] = 'Видимый';
$_lang['xmlwriter_err_nf'] = 'MODx requires the XMLWriter PHP extension to use this feature. Please ensure the extension is installed before continuing.';
