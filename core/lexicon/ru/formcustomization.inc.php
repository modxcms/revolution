...
$_lang['field'] = 'Поле';
$_lang['field_desc'] = 'Это область к которой применяется правило. Это может быть поле, вкладка или дополнительное поле. Если это дополнительное поле, то указывайте его в формате: &laquo;tv#&raquo;, где # это ID дополнительного поля.';
...
$_lang['profile_remove_confirm'] = 'Вы уверены, что хотите полностью удалить этот профиль? Это необратимо.';
...
$_lang['set_fields_msg'] = 'Здесь вы можете настроить поля для этой страницы, включая их видимость, метки и значения по умолчанию. Просто дважды щелкните на строке, чтобы изменить её значение, или оставьте поле пустым для использования значения по умолчанию. <br>Пожалуйста, учтите: скрывая элемент внутри этого профиля, он будет также скрыт в перекрывающихся профилях (даже если параметр указан как «видимый»).';
...
$_lang['set_msg'] = 'Здесь вы можете настроить стандартные поля, вкладки и дополнительные поля (TV) для страницы, к которой применяется этот набор правил.';
...
$_lang['set_tvs_msg'] = 'Здесь можно изменить настройки для дополнительных полей (TV) ресурса, их видимость, название, значение по умолчанию и указать вкладку, в которой они должны отображаться. Внимание: Если вы перемещаете дополнительное поле (TV) в другую вкладку, то порядок показа дополнительных полей (TV) можно настроить с помощью поля «Порядок сортировки».';
