<?php
/**
 * English language strings for Dashboards
 *
 * @package modx
 * @subpackage lexicon
 * @language en
 */
$_lang['dashboard'] = 'Kokpit';
$_lang['dashboard_add'] = 'Dodaj Kokpit';
$_lang['dashboard_create'] = 'Utwórz Kokpit';
$_lang['dashboard_desc_name'] = 'Nazwa Kokpitu.';
$_lang['dashboard_desc_description'] = 'Krótki opis Kokpitu.';
$_lang['dashboard_desc_hide_trees'] = 'Zaznacz, aby po zalogowaniu Kokpit był widoczny na całej szerokości ekranu.';
$_lang['dashboard_hide_trees'] = 'Ukryj drzewo po lewej stronie';
$_lang['dashboard_duplicate'] = 'Duplikuj Kokpit';
$_lang['dashboard_remove'] = 'Usuń Kokpit';
$_lang['dashboard_remove_confirm'] = 'Czy na pewno chcesz usunąć ten Kokpit?';
$_lang['dashboard_remove_multiple'] = 'Usuń Kokpit';
$_lang['dashboard_remove_multiple_confirm'] = 'Czy na pewno chcesz usunąć zaznaczone Kokpity?';
$_lang['dashboard_update'] = 'Edytuj Kokpit';
$_lang['dashboard_err_ae_name'] = 'Kokpit o nazwie "[[+name]]" już istnieje! Wpisz inną nazwę.';
$_lang['dashboard_err_duplicate'] = 'Wystąpił błąd podczas próby powielenia kokpitu.';
$_lang['dashboard_err_nf'] = 'Kokpitu nie znaleziono.';
$_lang['dashboard_err_ns'] = 'Nie określono Kokpitu.';
$_lang['dashboard_err_ns_name'] = 'Proszę podać nazwę widżetu.';
$_lang['dashboard_err_remove'] = 'Wystąpił błąd podczas próby usunięcia Kokpitu.';
$_lang['dashboard_err_remove_default'] = 'Nie można usunąć domyślnego Kokpitu!';
$_lang['dashboard_err_save'] = 'Wystąpił błąd podczas próby zapisania Kokpitu.';
$_lang['dashboard_usergroup_add'] = 'Przypisz Kokpit do Grupy Użytkowników';
$_lang['dashboard_usergroup_remove'] = 'Usuń przypisanie Kokpitu do Grupy';
$_lang['dashboard_usergroup_remove_confirm'] = 'Czy na pewno chcesz przypisać domyślny Kokpit tej Grupie Użytkowników?';
$_lang['dashboard_usergroups.intro_msg'] = 'Oto Grupy Użytkowników przypisane do tego Kokpitu.';
$_lang['dashboard_widget_err_placed'] = 'Ten widżet jest już umieszczony w tym Kokpicie!';
$_lang['dashboard_widgets.intro_msg'] = 'Tutaj można dodać, zarządzać i usuwać widżety z tego Kokpitu. Można również przeciągać i upuszczać wiersze w siatce, aby zmienić ich kolejność.';
$_lang['dashboards'] = 'Kokpity';
$_lang['dashboards.intro_msg'] = 'Here you can manage all the available Dashboards for this MODX manager.';
$_lang['rank'] = 'Ranga';
$_lang['user_group_filter'] = 'Po Grupie Użytkowników';
$_lang['widget'] = 'Widget';
$_lang['widget_content'] = 'Zawartość widżetu';
$_lang['widget_create'] = 'Utwórz nowy Widget';
$_lang['widget_err_ae_name'] = 'A widget with the name "[[+name]]" already exists! Please try another name.';
$_lang['widget_err_nf'] = 'Nie znaleziono widżetu!';
$_lang['widget_err_ns'] = 'Nie określono widżetu!';
$_lang['widget_err_ns_name'] = 'Proszę podać nazwę widżetu.';
$_lang['widget_err_remove'] = 'An error occurred while trying to remove the Widget.';
$_lang['widget_err_save'] = 'An error occurred while trying to save the Widget.';
$_lang['widget_file'] = 'Plik';
$_lang['widget_dashboards.intro_msg'] = 'Below is a list of all the Dashboards that this Widget has been placed on.';
$_lang['widget_dashboard_remove'] = 'Remove Widget From Dashboard';
$_lang['widget_description_desc'] = 'A description, or Lexicon Entry key, of the Widget and what it does.';
$_lang['widget_html'] = 'HTML';
$_lang['widget_lexicon_desc'] = 'The Lexicon Topic to load with this Widget. Useful for providing translations for the name and description, as well as any text in the widget.';
$_lang['widget_name_desc'] = 'The name, or Lexicon Entry key, of the Widget.';
$_lang['widget_new'] = 'Nowy widżet';
$_lang['widget_remove'] = 'Usuń Widżet';
$_lang['widget_remove_confirm'] = 'Are you sure you want to remove this Dashboard Widget? This is permanent, and will remove the Widget from all Dashboards.';
$_lang['widget_remove_multiple'] = 'Usuń wiele widżetów';
$_lang['widget_remove_multiple_confirm'] = 'Are you sure you want to remove these Dashboard Widgets? This is permanent, and will remove the Widgets from all their assigned Dashboards.';
$_lang['widget_namespace'] = 'Przestrzeń nazw';
$_lang['widget_namespace_desc'] = 'The Namespace that this widget will be loaded into. Useful for custom paths.';
$_lang['widget_php'] = 'Inline PHP Widget';
$_lang['widget_place'] = 'Umieść widget';
$_lang['widget_size'] = 'Rozmiar';
$_lang['widget_size_desc'] = 'The size of the widget. Can either be a half-screen wide ("Half"), the width of the screen ("Full"), or a full screen width and two rows ("Double").';
$_lang['widget_size_double'] = 'Podwójny';
$_lang['widget_size_full'] = 'Pełny';
$_lang['widget_size_half'] = 'Połowa';
$_lang['widget_snippet'] = 'Snippet';
$_lang['widget_type'] = 'Typ widżetu';
$_lang['widget_type_desc'] = 'The type of widget this is. "Snippet" widgets are MODX Snippets that are run and return their output. "HTML" widgets are just straight HTML. "File" widgets are loaded directly from files, which can either return their output or the name of the modDashboardWidgetClass-extended class to load. "Inline PHP" Widgets are widgets that are straight PHP in the widget content, similar to a Snippet.';
$_lang['widget_unplace'] = 'Remove Widget from Dashboard';
$_lang['widget_update'] = 'Aktualizuj widget';
$_lang['widgets'] = 'Widgety';
$_lang['widgets.intro_msg'] = 'Below is a list of all the installed Dashboard Widgets you have.';

$_lang['w_configcheck'] = 'Configuration Check';
$_lang['w_configcheck_desc'] = 'Displays a configuration check that ensures your MODX install is secure.';
$_lang['w_newsfeed'] = 'MODX News Feed';
$_lang['w_newsfeed_desc'] = 'Displays the MODX News Feed';
$_lang['w_recentlyeditedresources'] = 'Ostatnio edytowane';
$_lang['w_recentlyeditedresources_desc'] = 'Shows a list of the most recently edited resources by the user.';
$_lang['w_securityfeed'] = 'MODX Security Feed';
$_lang['w_securityfeed_desc'] = 'Displays the MODX Security Feed';
$_lang['w_whosonline'] = 'Aktywni użytkownicy';
$_lang['w_whosonline_desc'] = 'Shows a list of online users.';