        ...
        MODx.Ajax.request({
            ...
            ,listeners: {
                'success': {fn:function(response) {
                    ...
                    if (!this.windows.QuickEditFile) {
                        this.windows.QuickEditFile = MODx.load({
                            xtype: 'modx-window-file-quick-edit'
                            ,record: r
                        });
                    }
                    this.windows.QuickEditFile.reset();
                    this.windows.QuickEditFile.setValues(r);
                    this.windows.QuickEditFile.show(e.target);
                },scope:this}
            }
        });
    }
...
/** 
 * Generates the Quick Edit File window
 * 
 * @class MODx.window.QuickEditFile
 * @extends MODx.Window
 * @param {Object} config An object of configuration options.
 * @xtype modx-window-file-quick-edit
 */
MODx.window.QuickEditFile = function(config) {
    config = config || {};
    Ext.applyIf(config,{
    ...
    MODx.window.QuickEditFile.superclass.constructor.call(this,config);
};
Ext.extend(MODx.window.QuickEditFile,MODx.Window);
Ext.reg('modx-window-file-quick-edit',MODx.window.QuickEditFile);
