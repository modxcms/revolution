...
MODx.grid.Namespace = function(config) {
    ...
    Ext.applyIf(config,{
        ...
        ,baseParams: {
            action: 'Workspace/Namespaces/GetList'
        }
        ,fields: ['id','name','path','assets_path','perm']
        ,anchor: '100%'
        ,paging: true
        ,autosave: true
        ,save_action: 'Workspace/Namespaces/UpdateFromGrid'
        ,primaryKey: 'name'
        ,remoteSort: true
        ,sm: this.sm
        ,columns: [this.sm,{
...
Ext.extend(MODx.grid.Namespace,MODx.grid.Grid,{
        ...
        if (this.getSelectionModel().getCount() > 1) {
            ...
            if (p.indexOf('premove') != -1 && this.menu.record.name != 'core') {
                m.push({
                    ...
                    ,handler: this.remove.createDelegate(this,['namespace_remove_confirm','Workspace/Namespaces/Remove'])
                });
            }
        }
        return m;
    ...
    ,removeSelected: function() {
        ...
        MODx.msg.confirm({
            ...
            ,url: this.config.url
            ,params: {
                action: 'Workspace/Namespaces/RemoveMultiple'
                ,namespaces: cs
            }
            ,listeners: {
